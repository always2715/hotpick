...
const CURRENT_CONTENT_VERSION=126;
...
export default function Admin(props){
  ...
  const instagramItems=useMemo(()=>{
    ...
    (trends||[]).slice(0,30).forEach(item=>{if(item?.slug)map.set(item.slug,{...map.get(item.slug),...item,currentRank:item.rank});});
  ...
  async function backup(){const r=await fetch('/api/backup');if(!r.ok){setMessage('❌ 백업 오류');return;}const b=await r.blob(),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`stellate-v8.0.30-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href);}
  ...
  return <><Head><title>관리자 — STELLATE v8.0.30</title></Head><Header/><main className="admin-shell">
    <section className="admin-heading"><div><p className="eyebrow">STELLATE v8.0.30</p><h1>검증형 운영 관리자</h1></div><button className="admin-logout" onClick={logout}>로그아웃</button></section>
    ...
    {tab==='overview'&&<div className="admin-grid">
      ...
      <section className="admin-panel wide"><h2>빠른 작업</h2><p className="action-safety-note">비용 발생·대량 변경 작업은 확인 화면에서 내용을 검토한 뒤 실행됩니다.</p><div className="admin-actions"><button disabled={busy} onClick={()=>confirmAdminAction({action:'preview_trends'},'TOP 미리 계산을 완료했습니다.',{title:'TOP 후보를 미리 계산할까요?',description:'v8.0.20 관심도 기준으로 TOP 키워드 30개를 미리 계산하지만 실제 순위·피드·콘텐츠는 변경하지 않습니다.',impact:'검색·뉴스 API 호출은 발생하지만 콘텐츠 AI 생성은 실행하지 않습니다.',confirmLabel:'미리 계산 실행',tone:'warning',reload:true})}>TOP 미리 계산</button><button disabled={busy} onClick={()=>confirmAdminAction({action:'refresh_trends'},'TOP 원자적 갱신 작업을 등록했습니다.',{title:'검증된 TOP을 실제 적용할까요?',description:'관심도 기준으로 TOP 키워드 30개를 먼저 확정한 뒤, 같은 30개에 대한 상세·피드·요약정보·제목을 작성해 동시에 반영합니다.',impact:'외부 API와 AI 호출이 발생합니다. 31위 이하 후보로 교체하지 않으며, 확정된 같은 30개가 모두 준비될 때만 TOP·피드·상세를 교체합니다.',confirmLabel:'TOP 실제 적용',tone:'warning'})}>TOP 실제 적용</button><button disabled={busy} onClick={()=>confirmAdminAction({action:'refresh_trends_direct'},'TOP 후보 수집을 시작하고 QStash 소배치 처리를 등록했습니다.',{title:'TOP 갱신을 즉시 시작할까요?',description:'관심도 기준 TOP 키워드 30개를 확정하고, 해당 30개의 상세 조사를 3개씩 QStash 소배치로 처리합니다.',impact:'화면을 닫아도 배치가 계속되며 각 단계의 진행 위치가 Redis에 저장됩니다.',confirmLabel:'TOP 즉시 시작',tone:'warning'})}>TOP 즉시 시작</button>{activeRun&&<button className="danger-action" aria-label="현재 TOP 작업 즉시 중단" disabled={busy} onClick={()=>confirmAdminAction({action:'stop_active_trend_run',values:{runId:activeRun.runId},detail:'관리자 빠른 작업에서 현재 실행 즉시 중단'},'현재 TOP 갱신을 중단했습니다.',{title:'현재 TOP 작업을 즉시 중단할까요?',description:`${activeRun.runId} · 1차 처리 ${activeRun.attemptedCandidates||0}/${activeRun.total||30} · 준비 ${Number(activeRun.generated||0)+Number(activeRun.reused||0)}/${activeRun.total||30}`,impact:'실행 상태를 즉시 취소로 전환하고 대기·처리·추가검색 항목을 중단합니다. 이미 실행 중인 외부 호출은 반환 직후 폐기되며 다음 QStash 배치와 최종 공개는 차단됩니다. 기존 공개 TOP은 유지됩니다.',confirmLabel:'현재 TOP 즉시 중단',tone:'danger',reload:true})}>현재 TOP 작업 중단 (즉시)</button>}<button disabled={busy} onClick={()=>confirmAdminAction({action:'rebuild_missing_feeds'},'피드 인덱스 복구를 완료했습니다.',{title:'현재 공개 TOP 피드 목록을 다시 만들까요?',description:'피드 상세 콘텐츠는 유지하고, 현재 TOP30 원본에서 피드 목록·최신순·게시번호·조회수·카테고리 인덱스를 강제로 다시 만듭니다. 상세 콘텐츠 자체가 없는 항목만 생성 큐에 등록합니다.',impact:'오래된 인덱스 값이 남아 있어도 원본 콘텐츠와 대조해 덮어씁니다. 정상 상세 콘텐츠가 있으면 AI 호출 없이 복구하며, 상세 콘텐츠가 없는 항목에만 외부 API와 AI 호출이 발생할 수 있습니다.',confirmLabel:'피드 복구 실행',tone:'warning'})}>현재 TOP 피드 목록 재구성</button><button onClick={backup}>JSON 백업</button></div><div className="manual-row"><input value={keyword} onChange={e=>setKeyword(e.target.value)} placeholder="수동 키워드"/><button onClick={generateManual} disabled={busy||!keyword.trim()}>검증 콘텐츠 생성</button></div></section>
...
function RunRow({run,busy,confirmAdminAction}){
  ...
  const resumable=(run.executionStale||stopped)&&String(run.workflowType||'')==='top_refresh_v2';
