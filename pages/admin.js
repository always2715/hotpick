...
export default function Admin(props){
  ...
  async function backup(){const r=await fetch('/api/backup');if(!r.ok){setMessage('❌ 백업 오류');return;}const b=await r.blob(),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`stellate-v8.0.39-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href);}
  ...
  return <><Head><title>관리자 — STELLATE v8.0.39</title></Head><Header/><main className="admin-shell">
    <section className="admin-heading"><div><p className="eyebrow">STELLATE v8.0.39</p><h1>검증형 운영 관리자</h1></div><button className="admin-logout" onClick={logout}>로그아웃</button></section>
