...
assert.equal(CURRENT_TREND_ENGINE_VERSION,'8.0.56');
...
assert.equal(assessTrendRunCompatibility({engineVersion:'8.0.50',generationPoolCount:25,publicTopCount:20}).compatible,true);
assert.equal(assessTrendRunCompatibility({engineVersion:'8.0.51',generationPoolCount:25,publicTopCount:20}).compatible,true);
...
console.log('STELLATE v8.0.52 run compatibility and non-destructive feed recovery tests: PASS');
