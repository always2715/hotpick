...
// TOP은 새 실행에서 검증 완료된 30건이 모두 준비될 때만 교체합니다.
assert.match(refreshJob,/TARGET_TOP_COUNT\s*=\s*30/);
...
assert.match(kv,/ATOMIC_PUBLICATION_REQUIRES_TOP30/);
assert.match(kv,/ATOMIC_PUBLICATION_REQUIRES_30_CONTENTS/);
assert.match(kv,/stable_top30/);
assert.match(kv,/trends\.length!==30/);
...
assert.match(api,/source:'top30_fixed_keyword'/);
...
console.log('v8.0.12 atomic TOP30 and independent detail pipeline tests passed');
