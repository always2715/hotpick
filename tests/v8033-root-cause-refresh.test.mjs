...
const priorRunContent={slug:'kim-jong-un',candidateId:'r7-old',publicationStageId:'run-v8032:r7-old',contentVersion:128,keyword:'김정은',topKeyword:'김정은',displayTitle:'김정은'};
const migrated=assessStageIdentity(priorRunContent,trend,runId,1,129);
...
const sameRunConflict={...priorRunContent,publicationStageId:`${runId}:r99-other`,candidateId:'r99-other',contentVersion:129};
assert.equal(assessStageIdentity(sameRunConflict,trend,runId,1,129).matched,false,'현재 실행 안의 다른 후보 stage는 잘못 승격하면 안 됩니다.');
...
const serialized=JSON.stringify({...trend,contentVersion:129,blog:'검증 사실 기반 상세 콘텐츠'});
...
assert.match(jobsSource,/update-trends-v833/,'이전 QStash dedupe key와 분리해야 합니다.');
assert.match(versionSource,/contentVersion:\s*129/);
assert.match(versionSource,/trendCacheVersion:\s*49/);
