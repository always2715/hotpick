...
const rows=subjects.map((keyword,index)=>{
...
assert.equal(rows.filter(isResearchBackedFeedReady).length,30);
...
assert.ok(quality.uniqueTitleCount>=27);

const repeated=Array.from({length:30},(_,index)=>({...generic,topKeyword:`반복 ${index}`,topTitle:`반복 ${index} · 관련 관심 증가`}));
...
assert.match(job,/prepareSelectedTopCandidates\(\(prepared\.trends \|\| \[\]\)\.slice\(0, TARGET_TOP_COUNT\)/);
assert.match(job,/assessResearchBackedFeedSet/);
assert.match(job,/확정된 TOP30 키워드|같은 30개 키워드/);
...
console.log('STELLATE v8.0.25 fixed TOP30 compatibility tests: PASS');
