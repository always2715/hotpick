...
export function validateTopCandidateKeyword(value='') {
  ...
  if(/(?:오른|내린|올랐|내렸|답하는|질문하는|앞둔|나란히|개월만|경기만|전에는\s*꼭|징벌적)$/.test(keyword)||TRAILING_VERB_FRAGMENT.test(keyword))reasons.push('trailing_sentence_fragment');
  if(QUANTITY_ONLY_FRAGMENT.test(keyword))reasons.push('quantity_only_fragment');
