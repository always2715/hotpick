...
const KEYWORD_BOUNDARIES = new Set([
  ...
  '입대','전역','이적','영입','계약','부상','결장','출전','득점','컴백','신곡','앨범','영화','드라마',
...
const ENTITY_TRAILING_NOISE = new Set([
  ...KEYWORD_BOUNDARIES,
  '전국투어','투어','팬미팅','예매','티켓','일정','장소','근황','입장','거취','보도','인터뷰','차기작','주연',
...
function keywordFragments(value='') {
  ...
  for(const segment of splitEntitySegments(value)){
    ...
    for(let index=0;index<words.length;index++){
      ...
      if(KEYWORD_BOUNDARIES.has(token)||ENTITY_TRAILING_NOISE.has(token)||/(?:했다|한다|됐다|된다|입니다|합니다|인가요|일까요)$/.test(token)){
        boundary=index;
        break;
      }
    }
...
export function resolveTop30Keyword(input={}) {
  ...
  const winner=scored[0];
  return winner
    ? {ok:true,keyword:winner.keyword,source:winner.source,score:Number(winner.score.toFixed(2)),candidates:scored.slice(0,12)}
    : resolveEditorialKeyword(input);
}
...
export function derivePostResearchTitle(keyword='',pkg={},ledger={},sourceTitles=[],options={}) {
  ...
  const keywordResult=options.fixedKeyword===true
    ? {ok:Boolean(cleanHeadline(keyword)),keyword:cleanHeadline(keyword).slice(0,30),source:'top30_fixed_keyword',candidates:[cleanHeadline(keyword).slice(0,30)].filter(Boolean)}
  ...
  const add=(value,source,base)=>{
    ...
    candidates.push({event:quality.event,source,score:base+support*20});
  };

  add(pkg.shortTitle||pkg.coreEvent||'','ai_after_detail',120);
  for(const value of candidateRowsFromDetail(detailContent,selectedKeyword))add(value,'detail_content',100);
  for(const fact of facts.filter(fact=>fact.status==='confirmed'||fact.sourceType==='official'))add(extractEventPhraseFromText(fact.text,selectedKeyword),'fact_ledger_fallback',110);
  for(const fact of facts.filter(fact=>fact.status!=='confirmed'&&fact.sourceType!=='official'))add(extractEventPhraseFromText(fact.text,selectedKeyword),'fact_ledger_fallback',85);
  ...
  return {
    ...
    titleStatus:'ready',titleReady:true,titleSource:winner.source,titleValidationReasons:[],
    keywordSource:keywordResult.source||'unresolved',keywordCandidates:keywordResult.candidates||[],
  };
}
