...
export function derivePostResearchTitle(keyword='',pkg={},ledger={},sourceTitles=[],options={}) {
  ...
  const keywordResult=options.fixedKeyword===true
    ? {ok:Boolean(cleanHeadline(keyword)),keyword:cleanHeadline(keyword).slice(0,30),source:'top20_fixed_keyword',candidates:[cleanHeadline(keyword).slice(0,30)].filter(Boolean)}
