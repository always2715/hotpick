...
const KEYWORD_BOUNDARIES = new Set([
  ...
  '화재','폭발','사망','사고','누출','대피','구조','진압','이유','전망','정리','소식','상황','동향','논란','화제'
]);
...
function stripParticle(value='') {
  ...
  // v8.0.55: 기사 제목에서 기업명 뒤에 붙은 축약 조사(예: SK스페셜티서)를
  // 고유명사의 일부로 저장하지 않습니다. 영문·숫자가 섞인 긴 토큰에만 제한 적용합니다.
  if(token.length>=4&&/[A-Za-z0-9]/.test(token)&&token.endsWith('서'))token=token.slice(0,-1);
...
export function validateEditorialKeyword(value='') {
  const keyword=cleanHeadline(value).replace(/\s+·\s+.*/,'').trim().split(/\s+/).map(token=>token.length>=4&&/[A-Za-z0-9]/.test(token)&&token.endsWith('서')?token.slice(0,-1):token).filter(Boolean).join(' ');
  const reasons=[];
  const tokens=tokenize(keyword);
  ...
  if(stripped.length&&stripped.every(token=>KEYWORD_BOUNDARIES.has(token)||GENERIC_KEYWORDS.has(token)||GENERIC_ENTITY_SINGLES.has(token.toLowerCase())))reasons.push('event_only');
  if(stripped.length===1&&GENERIC_ENTITY_SINGLES.has(stripped[0].toLowerCase()))reasons.push('generic_singleton');
  return {valid:reasons.length===0,keyword,reasons};
}

const INVALID_TOP_LEADING = /^(?:나란히|연속|질문(?:에|을)?|답하는|오른|내린|앞둔|무려|결국|단독|속보|기자수첩|사설|칼럼|취재수첩|인터뷰)(?:\s|$)/;
...
const TRAILING_MODIFIER_TOKEN = /^(?:한|하는|하려는|하려고|했던|된|되는|될|인|이라는|같은|없는|있는|위한|대한|놓고|두고|둘러싼|앞둔|본격화)$/;
...
export function validateTopCandidateKeyword(value='') {
  ...
  const rawTokens=keyword.split(/\s+/).filter(Boolean);
  if(rawTokens.length>=2&&TRAILING_MODIFIER_TOKEN.test(rawTokens.at(-1)))reasons.push('trailing_modifier_fragment');
  if(/^(?:기자수첩|사설|칼럼|취재수첩|인터뷰)\b/.test(keyword))reasons.push('editorial_label_fragment');
  const tokens=tokenize(keyword);
