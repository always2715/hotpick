...
import { compactTopPreviewContent } from './topPreviewPolicy.js';
...
function informativeRow(value='') {
  const text=clean(value);
  if(text.length<18)return false;
  if(/^(?:관련|이번|해당|현재) (?:내용|상황|이슈)/.test(text))return false;
  if(/관심이 (?:커지고|높아지고)|귀추가 주목|지켜볼 필요|의미가 있습니다|중요한 변화/.test(text))return false;
  return true;
}

function eventPriority(value='') {
  const text=clean(value);let score=0;
  if(/\d{1,4}(?:년|월|일|시|분|%|원|명|건|회|위|점|경기)/.test(text))score+=3;
  if(/발표|공개|확정|결정|출시|시행|개최|승리|패배|득점|복구|중단|기소|판결|선임|사임/.test(text))score+=4;
  if(/예정|계획|신청|예매|예약|대상|조건|기준/.test(text))score+=2;
  return score;
}
...
export function buildFeedSummaryCard({keyword='', feedTitle='', blog='', sections=[], factLedger={}}={}) {
  ...
  const issueSentences=uniqueRows((issueSection.paragraphs||[]).flatMap(sentences)).filter(row=>!isGenericFactText(row)&&informativeRow(row)).sort((a,b)=>eventPriority(b)-eventPriority(a));
  const contextSentences=uniqueRows(contextSections.flatMap(section=>(section.paragraphs||[]).flatMap(sentences))).filter(row=>!isGenericFactText(row)&&informativeRow(row));
  const insightSentences=uniqueRows((insightSection.paragraphs||[]).flatMap(sentences)).filter(row=>!isGenericFactText(row));
  const ledgerFacts=accurateFacts(factLedger,{scope:'issue',limit:8,allowSingleTrusted:true}).map(row=>clean(row.text||row.claim)).filter(informativeRow).sort((a,b)=>eventPriority(b)-eventPriority(a));
  ...
  const preview=compactTopPreviewContent({summary:lead,why:context,points:points.slice(0,5)});
  return {
    previewLabel:'요약 정보',
    ...
    why:preview.why,
    summaryParagraphs:[preview.summary,preview.why].filter(Boolean),
    listSummary:trimSentence([preview.summary,preview.why].filter(Boolean).join(' '),100),
    points:preview.points,
    previewCharacterCount:preview.characterCount,
    previewMaxChars:preview.maxChars,
    source:'feed_summary_v6_precise_editorial',
