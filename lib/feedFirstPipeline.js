...
export function buildFeedSummaryCard({keyword='', feedTitle='', blog='', sections=[], factLedger={}}={}) {
  ...
  const basic=structured[0]?.paragraphs||[];
  const issues=structured[1]?.paragraphs||[];
  const insight=structured.find(section=>/STELLATE\s*인사이트/i.test(section.heading))?.paragraphs||structured[2]?.paragraphs||[];
  const issueSentences=uniqueRows(issues.flatMap(sentences)).filter(row=>!isGenericFactText(row));
  const basicSentences=uniqueRows(basic.flatMap(sentences)).filter(row=>!isGenericFactText(row));
  const insightSentences=uniqueRows(insight.flatMap(sentences)).filter(row=>!isGenericFactText(row));
  const ledgerFacts=accurateFacts(factLedger,{scope:'issue',limit:5,allowSingleTrusted:true}).map(row=>clean(row.text||row.claim)).filter(Boolean);
  const profileLedgerFacts=accurateFacts(factLedger,{scope:'profile',limit:3,allowSingleTrusted:true}).map(row=>clean(row.text||row.claim)).filter(Boolean);

  const summaryFirst=issueSentences[0]||ledgerFacts[0]||intro[0]||basicSentences[0]||profileLedgerFacts[0]||'';
  const secondaryCandidates=uniqueRows([
    ...issueSentences.slice(1,3),
    ...ledgerFacts.slice(1,3),
    ...basicSentences.slice(1,2),
    ...profileLedgerFacts.slice(1,2),
  ]).filter(row=>clean(row)!==clean(summaryFirst));
  const summary=trimSentence(summaryFirst,180);
  const why=secondaryCandidates.length?trimSentence(secondaryCandidates.slice(0,2).join(' '),220):'';

  const pointCandidates=uniqueRows([
    ...issueSentences.slice(0,5),
    ...ledgerFacts.slice(0,5),
  ]).map(row=>compactPoint(row,80)).filter(Boolean);
  const points=uniqueRows(pointCandidates).slice(0,5);
  for(const fallback of [summary,why,...basicSentences]){
    if(points.length>=3)break;
    const point=compactPoint(fallback,80);
  ...
  return {
    ...
    summary,
    why,
    summaryParagraphs:[summary,why].filter(Boolean),
    listSummary:trimSentence(`${summary}${why?` ${why}`:''}`,100),
    points:points.slice(0,5),
    source:'feed_summary_v4_accuracy_first',
