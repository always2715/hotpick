...
export function contentLengthRange(grade='D') {
  const key=String(grade||'D').toUpperCase();
  if(key==='A')return {min:900,max:1600};
  if(key==='B')return {min:600,max:1000};
  if(key==='C')return {min:320,max:650};
  return {min:160,max:700};
}
