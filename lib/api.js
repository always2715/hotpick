...
export const CONTENT_VERSION = 136;
...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.47 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.47 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.47; +https://stellate.co.kr)',
...
- 모든 출력은 존댓말로 작성하세요.`;
  ...
  // v8.0.47: TOP 단건 생성은 fastRefresh라 일반 품질 수정 단계를 건너뛰므로,
...
export async function getCachedContent(slug, keyword, preferredImage = null, trendMeta = {}, options = {}) {
  ...
  try {
    ...
    // v8.0.47: 최종 공개 검증 전에도 실행별 원본을 먼저 남깁니다.
