...
- 모든 출력은 존댓말로 작성하세요.`;
  ...
  // v8.0.55: 1,000자 미달을 다음 전체 검색 시도로 넘기지 않고, 같은 Fact Ledger와 같은 요청 안에서
  // 본문 구조만 한 번 보강합니다. 새로운 사실·전망·일반론을 추가할 수 없고 정확성 검증을 다시 통과해야 합니다.
  if(rendered.blog.length<FEED_DETAIL_MIN_CHARS&&!costGuardBlocked&&usableClaudeKey&&!claudeAuthUnavailable){
    const beforeLength=rendered.blog.length;
    const lengthRepairPrompt=`${prompt}

[분량 미달 전용 수정]
현재 JSON은 검증 Fact를 사용했지만 상세 본문이 ${beforeLength}자로 최소 ${FEED_DETAIL_MIN_CHARS}자에 미달했습니다. 외부 검색을 다시 하지 말고 아래 JSON의 전체 구조만 다시 작성하세요.
- 제공된 Fact ID 외 사실, 원인, 영향, 전망, 평가를 추가하지 마세요.
- 같은 Fact를 문장만 바꿔 반복하지 마세요.
- 서로 직접 연결되는 Fact를 주체·행동·시점·수치·조건 순으로 충분히 설명하세요.
- 확인 Fact가 실제로 부족해 최소 분량을 만들 수 없으면 억지로 채우지 말고 sections를 사실 범위에서만 작성하세요.
- JSON 전체를 출력하세요.
...
${JSON.stringify(pkg)}`;
    try{
      ...
      if(repairedPkg){
        const repairedAccuracy=validateGeneratedPackageAccuracy(repairedPkg,factLedger);
        const repairedRendered=renderBlogPackage(repairedPkg,onlineTrend,{keyword:topicTitle,eventTitle:repairedPkg.shortTitle||''});
        if(repairedAccuracy.passed&&repairedRendered.blog.length>rendered.blog.length){
          pkg=repairedPkg;rendered=repairedRendered;aiStatus='claude_length_repair';
          aiError=aiError||`TARGETED_LENGTH_REPAIR:${beforeLength}->${rendered.blog.length}`;
        }
      }
    }catch(error){
      console.warn(`[targeted length repair skipped] ${keyword}:`,error?.message||error);
    }
  }
  // AI 보강이 불가능하거나 더 짧을 때는 저장된 구체 Fact를 중복 없이 최대한 사용하는 결정론적 문안과 비교합니다.
  if(rendered.blog.length<FEED_DETAIL_MIN_CHARS){
    ...
    if(supportedAccuracy.passed&&supportedRendered.blog.length>rendered.blog.length){
      ...
      aiError=aiError||`DETERMINISTIC_LENGTH_RECOVERY:${beforeLength}->${rendered.blog.length}`;
    }
  }
