...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.41 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.41 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.41; +https://stellate.co.kr)',
