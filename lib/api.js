...
import { FEED_DETAIL_MIN_CHARS, FEED_DETAIL_TARGET_CHARS, FEED_DETAIL_RECOMMENDED_MIN_CHARS, FEED_DETAIL_RECOMMENDED_MAX_CHARS, feedDetailLength } from './feedLengthPolicy.js';
...
export const CONTENT_VERSION = 140;
...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.56 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.56 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
...
function buildProfileEvidence(items=[],topicTitle='') {
  ...
  return (Array.isArray(items)?items:[]).map(item=>{
    const link=canonicalizeUrl(item.link||'');
    const structuredFacts=(Array.isArray(item.facts)?item.facts:[]).map(row=>cleanText(row?.text||row)).filter(Boolean).join(' ');
    const text=cleanText(`${item.title||''} ${item.description||''} ${structuredFacts}`);
    ...
    return normalizeSourceItem({...item,description:cleanText(`${item.description||''} ${structuredFacts}`).slice(0,1200),sourceType,rightsBasis:'profile_search_metadata_fact_verification',evidenceUsable:true,contentUsable:true,discoveryOnly:false,bodyFetchAllowed:false,textReuseAllowed:false,provider:'profile_metadata',evidenceScope:'profile'});
  }).filter(item=>{
...
function attachProfileFacts(ledger={},profileEvidence=[],topicTitle='') {
  ...
  (Array.isArray(profileEvidence)?profileEvidence:[]).forEach((item,index)=>{
    ...
    const trustedProfileSource=['official','authorized','trusted_news'].includes(String(item.sourceType||''));
    const topicTokens=cleanText(topicTitle).toLowerCase().split(/\s+/).filter(token=>token.length>1);
    const sentences=cleanText(`${item.description||''} ${item.title||''}`).split(/(?<=[.!?다요])\s+|[•·]\s*/).map(cleanText)
      ...
      .filter(text=>topicTokens.length===0||topicTokens.some(token=>text.toLowerCase().includes(token)))
      .filter(text=>trustedProfileSource||/가수|배우|방송인|선수|감독|목사|의사|정치인|기업|회사|제품|서비스|작품|프로그램|도시|지역|기관|협회|위원회|그룹|팀|브랜드|암호화폐|가상자산|활동|데뷔|소속|출신|장르|기능|특징|설립|운영|정의|의미/.test(text))
      .slice(0,3);
...
export async function researchTopic(keyword, context={}) {
  ...
  // v8.0.56: 36시간 이슈 근거와 별도로 공식·공인 기본정보는 프로필 근거로 재사용합니다.
  const profileEvidence=buildProfileEvidence([...officialEvidence,...profileLinks],topicTitle);
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.56; +https://stellate.co.kr)',
...
export function assessContentQuality(content) {
  ...
  const length=feedDetailLength(content);
  ...
  if(length<minimumLength){score-=35;reasons.push(`피드 공개 텍스트 최소 ${minimumLength.toLocaleString()}자 미만`);}
...
const CONTENT_OUTPUT_SCHEMA={
  ...
  properties:{
    ...
    sections:{type:'array',minItems:3,maxItems:5,items:{type:'object',additionalProperties:false,properties:{heading:{type:'string'},paragraphs:{type:'array',minItems:1,items:{type:'object',additionalProperties:false,properties:{text:{type:'string'},claimIds:{type:'array',items:{type:'string'}}},required:['text','claimIds']}}},required:['heading','paragraphs']}},
    uncertainties:{type:'array',items:{type:'string'}},
  },
  required:['visualQuery','sections','uncertainties'],
};
...
function trimFeedDerivedAnswer(value='',max=220){
  const text=sanitizePublicText(cleanText(value));
  if(text.length<=max)return text;
  const sentences=text.split(/(?<=[.!?다요])\s+/).filter(Boolean);
  let out='';
  for(const sentence of sentences){const next=`${out} ${sentence}`.trim();if(next.length>max)break;out=next;}
  return (out||text).slice(0,max).trim();
}

function buildFeedDerivedQa(keyword='',rendered={},limit=3){
  const sections=Array.isArray(rendered?.sections)?rendered.sections:[];
  const rows=[];
  for(const [index,section] of sections.entries()){
    const paragraph=(Array.isArray(section?.paragraphs)?section.paragraphs:[]).find(row=>cleanText(row?.text||row).length>=20);
    if(!paragraph)continue;
    const heading=cleanText(section.heading||'현재 확인된 내용');
    const q=index===0?`${keyword}에 대해 확인된 기본 내용은 무엇인가요?`:index===1?`${heading}의 핵심은 무엇인가요?`:`${heading}에서 확인할 내용은 무엇인가요?`;
    rows.push({q:sanitizePublicText(q).slice(0,100),a:trimFeedDerivedAnswer(paragraph?.text||paragraph,220),claimIds:Array.isArray(paragraph?.claimIds)?paragraph.claimIds:[]});
    if(rows.length>=limit)break;
  }
  return rows.filter(row=>row.q&&row.a);
}

function feedDerivedTitleFallback(keyword='',rendered={},derived={}){
  if(derived?.titleReady===true&&derived?.topTopic)return derived;
  const generic=/^(?:현재 확인된 주요 내용|핵심 맥락과 확인 포인트|이용자가 알아둘 점|STELLATE 인사이트|기본정보와 주요 특징|주요 활동 정보)$/;
  const headings=(Array.isArray(rendered?.sections)?rendered.sections:[]).map(row=>cleanText(row?.heading||'')).filter(Boolean);
  let topic=headings.slice(1).find(row=>row.length>=4&&!generic.test(row))||headings.find(row=>row.length>=4&&!generic.test(row))||'핵심 사실 정리';
  const escaped=String(keyword||'').replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  if(escaped)topic=topic.replace(new RegExp(`^${escaped}(?:은|는|이|가)?\\s*`),'').trim();
  topic=topic.replace(/[.!?]+$/,'').replace(/^(?:현재|최근|주요|관련|핵심)\s+/,'').trim().slice(0,18);
  if(topic.length<4||generic.test(topic))topic='핵심 사실 정리';
  return {...derived,topKeyword:keyword,topTopic:topic,shortTitle:topic,topTitle:`${keyword} · ${topic}`.slice(0,64),displayTitle:keyword,titleStatus:'ready',titleReady:true,titleSource:'feed_heading_fallback',titleValidationReasons:[],keywordSource:derived?.keywordSource||'fixed_ranked_keyword_verified'};
}


// v8.0.56 feed-first pipeline:
// 1) 검증된 피드 본문 완성 → 2) 피드 요약 생성 → 3) 피드 기반 제목 확정
// → 4) Q&A·인스타 카드 파생. 보조 콘텐츠는 피드 정확성 판정을 거꾸로 실패시키지 않습니다.
function buildFeedFirstDerivedOutputs({
  topicTitle='', pkg={}, rendered={}, factLedger={}, evidenceItems=[], trendMeta={},
  sourceKeyword='', requestedTopTitle='', identityMode=false, isFullTier=true,
  publishableFacts=[], contentTier='full',
}={}) {
  // 실제 요약정보 카드는 전체 피드 작성 후 생성하고, 제목은 그 다음 완성 피드를 기준으로 확정합니다.
  const provisionalCard=buildFeedSummaryCard({
    keyword:topicTitle,
    feedTitle:topicTitle,
    blog:rendered.blog,
    sections:rendered.sections,
    factLedger,
  });

  let editorialTop=identityMode
    ? {
        topKeyword:topicTitle,
        topTopic:pkg.shortTitle||'기본정보와 주요 특징',
        topTitle:pkg.topTitle||`${topicTitle} · ${pkg.shortTitle||'기본정보와 주요 특징'}`,
        displayTitle:topicTitle,
        shortTitle:pkg.shortTitle||'기본정보와 주요 특징',
        titleStatus:'ready',titleReady:true,titleSource:'keyword_identity_fallback',
        titleValidationReasons:[],keywordSource:'fixed_top20_keyword',
      }
    : derivePostResearchTitle(topicTitle,pkg,factLedger,evidenceItems.map(x=>x.title),{
        detailContent:rendered.factualBlog,
        rawKeyword:trendMeta.rawKeyword||sourceKeyword,
        displayTitle:requestedTopTitle,
        fixedKeyword:true,
      });
  editorialTop=feedDerivedTitleFallback(topicTitle,rendered,editorialTop);

  const feedHeadline=feedHeadlineFromTitle(
    editorialTop.topKeyword||topicTitle,
    editorialTop.topTopic||editorialTop.topTitle,
    '관련 관심 증가',
  );
  const feedTitle=fullFeedTitle(editorialTop.topKeyword||topicTitle,feedHeadline);
  const detailTitle=normalizeDetailTitle(
    editorialTop.topKeyword||topicTitle,
    '',
    feedTitle,
    evidenceItems.map(item=>item.title),
  );
  const card={...provisionalCard,feedTitle,detailTitle};

  const validIds=new Set((Array.isArray(publishableFacts)?publishableFacts:[]).map(f=>String(f.id)));
  const sanitizeClaims=ids=>(Array.isArray(ids)?ids:[]).map(String).filter(id=>validIds.has(id));
  const claimMap=(Array.isArray(rendered.claimMap)?rendered.claimMap:[])
    .map(row=>({...row,claimIds:sanitizeClaims(row.claimIds)}));
  const qa=isFullTier
    ? buildFeedDerivedQa(editorialTop.topKeyword||topicTitle,rendered,3)
        .map(row=>({...row,claimIds:sanitizeClaims(row.claimIds)}))
    : [];
  const instagramCards=isFullTier
    ? normalizeInstagramCards(topicTitle,card,evidenceItems,[],{
        blog:rendered.blog,claimMap,factLedger,sourceItems:evidenceItems,
        feedTitle,topTitle:editorialTop.topTitle,displayTitle:topicTitle,keyword:topicTitle,
      })
    : [];

  return {editorialTop,feedHeadline,feedTitle,detailTitle,card,claimMap,qa,instagramCards,contentTier};
}
...
export async function generateContent(keyword, newsBundle, existingContent = null, preferredImage = null, trendMeta = {}) {
  ...
  const tierRules='- 출력 JSON에는 visualQuery, sections, uncertainties만 포함하세요. 제목·요약·Q&A·인스타 카드는 출력하지 마세요.';
...
- 각 본문 문단에는 근거 claimIds를 넣으세요. 요약·Q&A·인스타 카드는 검증된 피드에서 시스템이 나중에 파생합니다.
...
- 본문·요약·포인트·Q&A에는 언론사명, 출처 도메인, 기자명, 원문 확인 안내를 넣지 마세요. 단, 언론사나 기자가 사건의 직접 주체인 경우에만 Fact에 있는 범위로 언급할 수 있습니다. 출처 목록은 시스템이 별도 영역에 표시합니다.
- ‘출처에서 확인할 수 있습니다’, ‘자료를 기준으로 정리했습니다’, ‘동명이인과 섞이지 않도록 구분했습니다’, ‘공개 자료에 없는 내용은 확정하지 않았습니다’, ‘원문과 후속 공지를 확인하세요’처럼 작성·검증 절차를 설명하는 문장은 독자용 정보가 아니므로 절대 출력하지 마세요.
- 기사나 언론의 객관성·정보 제공 기능을 평가하는 문장을 사실처럼 옮기지 마세요. 해당 평가 자체가 이 주제의 핵심 사건이고 별도 Fact로 교차 확인된 경우가 아니면 제외하세요.
...
- 먼저 전체 상세 내용을 작성한 뒤 최종 TOP 제목은 시스템이 완성된 피드와 Fact Ledger를 다시 읽어 4~18자의 구체 사건명으로 생성합니다. 제목·요약을 먼저 만들지 마세요.
- 피드 요약, 포인트, Q&A, 인스타 카드는 시스템이 완성된 피드에서만 파생합니다.
${tierRules}
...
- 모든 출력은 존댓말로 작성하세요.`;
  ...
  if(aiStatus==='claude'){
    const strictAccuracy=validateGeneratedPackageAccuracy(pkg,factLedger,{scope:'feed'});
  ...
  let derivedOutputs=buildFeedFirstDerivedOutputs({topicTitle,pkg,rendered,factLedger,evidenceItems,trendMeta,sourceKeyword,requestedTopTitle,identityMode,isFullTier,publishableFacts,contentTier});
  let editorialTop=derivedOutputs.editorialTop;
  let feedHeadline=derivedOutputs.feedHeadline;
  let normalizedFeedTitle=derivedOutputs.feedTitle;
  let normalizedDetailTitle=derivedOutputs.detailTitle;
  let normalizedCard=derivedOutputs.card;
  let claimMap=derivedOutputs.claimMap;
  let qa=derivedOutputs.qa;
  let instagramCards=derivedOutputs.instagramCards;
  ...
  // v8.0.56: 분량 복구 여부도 본문 단독이 아니라 실제 피드 화면의 최종 공개 텍스트로 판정합니다.
  // 메인 제목·요약·포인트·소제목·본문·Q&A·표시 공백을 모두 합산한 뒤, 미달할 때만
  // 같은 Fact Ledger를 이용해 피드 본문 구조를 한 번 보강합니다.
  if(feedDetailLength(draft)<FEED_DETAIL_MIN_CHARS&&!costGuardBlocked&&usableClaudeKey&&!claudeAuthUnavailable){
    const beforeLength=feedDetailLength(draft);
    const lengthRepairPrompt=`${prompt}

[최종 공개 피드 분량 보강]
현재 완성 피드의 메인 제목·요약·포인트·소제목·본문·Q&A·표시 공백을 합산한 분량은 ${beforeLength}자입니다. 외부 검색을 다시 하지 말고 제공된 Fact Ledger만 이용해 sections를 다시 작성하세요.
- 피드 구성 요건과 소제목 구조는 그대로 유지하세요.
- 제공된 Fact ID 외 사실, 원인, 영향, 전망, 평가를 추가하지 마세요.
- 같은 Fact를 문장만 바꿔 반복하지 마세요.
- 서로 직접 연결되는 Fact를 주체·행동·대상·시점·수치·조건 순으로 충분히 설명하세요.
- 확인 Fact가 실제로 부족하면 억지 문장을 추가하지 마세요.
- visualQuery, sections, uncertainties만 포함한 JSON 전체를 출력하세요.

[현재 JSON]
${JSON.stringify(pkg)}`;
    try{
      const repairedPkg=await callClaudeJSON(lengthRepairPrompt,Math.max(4200,Math.min(6200,Number(guard.maxOutput||5200))),null);
      if(repairedPkg){
        const repairedAccuracy=validateGeneratedPackageAccuracy(repairedPkg,factLedger,{scope:'feed'});
        const repairedRendered=renderBlogPackage(repairedPkg,onlineTrend,{keyword:topicTitle,eventTitle:repairedPkg.shortTitle||''});
        const repairedDerived=buildFeedFirstDerivedOutputs({topicTitle,pkg:repairedPkg,rendered:repairedRendered,factLedger,evidenceItems,trendMeta,sourceKeyword,requestedTopTitle,identityMode,isFullTier,publishableFacts,contentTier});
        const repairedDraft={blog:repairedRendered.blog,claimMap:repairedDerived.claimMap,card:repairedDerived.card,qa:repairedDerived.qa,instagramCards:repairedDerived.instagramCards,feedTitle:repairedDerived.feedTitle,detailTitle:repairedDerived.detailTitle,topTitle:repairedDerived.editorialTop.topTitle,titleStatus:repairedDerived.editorialTop.titleStatus,titleReady:repairedDerived.editorialTop.titleReady,contentTier};
        const repairedLength=feedDetailLength(repairedDraft);
        if(repairedAccuracy.passed&&repairedLength>beforeLength){
          pkg=repairedPkg;rendered=repairedRendered;derivedOutputs=repairedDerived;
          editorialTop=repairedDerived.editorialTop;feedHeadline=repairedDerived.feedHeadline;
          normalizedFeedTitle=repairedDerived.feedTitle;normalizedDetailTitle=repairedDerived.detailTitle;
          normalizedCard=repairedDerived.card;claimMap=repairedDerived.claimMap;qa=repairedDerived.qa;instagramCards=repairedDerived.instagramCards;
          visualQuery=String(repairedPkg.visualQuery||visualQuery||'').toLowerCase().replace(/[^a-z\s-]/g,' ').replace(/\s+/g,' ').trim().split(' ').slice(0,4).join(' ');
          draft=repairedDraft;aiStatus='claude_length_repair';aiError=aiError||`TARGETED_PUBLIC_LENGTH_REPAIR:${beforeLength}->${repairedLength}`;
        }
      }
    }catch(error){
      console.warn(`[targeted public length repair skipped] ${keyword}:`,error?.message||error);
    }
  }

  // AI 보강이 불가능하거나 더 짧을 때는 같은 Fact를 직접 투영한 결정론적 피드와
  // 최종 공개 텍스트 길이를 비교합니다. 더 길고 정확한 경우에만 교체합니다.
  if(feedDetailLength(draft)<FEED_DETAIL_MIN_CHARS){
    const beforeLength=feedDetailLength(draft);
    const supportedPkg=buildVerifiedFallback(topicTitle,factLedger,sourceWindowHours,contentTier);
    const supportedAccuracy=validateGeneratedPackageAccuracy(supportedPkg,factLedger,{scope:'feed'});
    const supportedRendered=renderBlogPackage(supportedPkg,onlineTrend,{keyword:topicTitle,eventTitle:supportedPkg.shortTitle||''});
    const supportedDerived=buildFeedFirstDerivedOutputs({topicTitle,pkg:supportedPkg,rendered:supportedRendered,factLedger,evidenceItems,trendMeta,sourceKeyword,requestedTopTitle,identityMode,isFullTier,publishableFacts,contentTier});
    const supportedDraft={blog:supportedRendered.blog,claimMap:supportedDerived.claimMap,card:supportedDerived.card,qa:supportedDerived.qa,instagramCards:supportedDerived.instagramCards,feedTitle:supportedDerived.feedTitle,detailTitle:supportedDerived.detailTitle,topTitle:supportedDerived.editorialTop.topTitle,titleStatus:supportedDerived.editorialTop.titleStatus,titleReady:supportedDerived.editorialTop.titleReady,contentTier};
    const supportedLength=feedDetailLength(supportedDraft);
    if(supportedAccuracy.passed&&supportedLength>beforeLength){
      pkg=supportedPkg;rendered=supportedRendered;derivedOutputs=supportedDerived;
      editorialTop=supportedDerived.editorialTop;feedHeadline=supportedDerived.feedHeadline;
      normalizedFeedTitle=supportedDerived.feedTitle;normalizedDetailTitle=supportedDerived.detailTitle;
      normalizedCard=supportedDerived.card;claimMap=supportedDerived.claimMap;qa=supportedDerived.qa;instagramCards=supportedDerived.instagramCards;
      draft=supportedDraft;aiStatus='verified_length_recovery';aiError=aiError||`DETERMINISTIC_PUBLIC_LENGTH_RECOVERY:${beforeLength}->${supportedLength}`;
    }
  }

  let preliminaryQuality=assessContentQuality({...draft,sourceItems:evidenceItems,factLedger});
  let preliminaryCopyright=assessCopyrightRisk(draft,copyrightSources,{scope:'feed'});
  let preliminaryGrounding=assessGrounding(draft,factLedger,{scope:'feed'});
  ...
  if(aiStatus==='claude'&&guard.allowRevision&&trendMeta.fastRefresh!==true&&(preliminaryQuality.score<82||!preliminaryCopyright.passed||preliminaryGrounding.score<90)){
    ...
    try{
      ...
      if(revisedPkg){
        ...
        const revisedDerived=buildFeedFirstDerivedOutputs({topicTitle,pkg:revisedPkg,rendered:revisedRendered,factLedger,evidenceItems,trendMeta,sourceKeyword,requestedTopTitle,identityMode,isFullTier,publishableFacts,contentTier});
        const revisedEditorialTop=revisedDerived.editorialTop;
        const revisedHeadline=revisedDerived.feedHeadline;
        const revisedFeed=revisedDerived.feedTitle;
        const revisedDetail=revisedDerived.detailTitle;
        const revisedVisual=String(revisedPkg.visualQuery||'').toLowerCase().replace(/[^a-z\s-]/g,' ').replace(/\s+/g,' ').trim().split(' ').slice(0,4).join(' ');
        const revisedCard=revisedDerived.card;
        const revisedClaimMap=revisedDerived.claimMap;
        const revisedQa=revisedDerived.qa;
        const revisedInstagram=revisedDerived.instagramCards;
        ...
        const candidateCopyright=assessCopyrightRisk(candidate,copyrightSources,{scope:'feed'});
        const candidateGrounding=assessGrounding(candidate,factLedger,{scope:'feed'});
        ...
        const revisedAccuracy=validateGeneratedPackageAccuracy(revisedPkg,factLedger,{scope:'feed'});
  ...
  if(!preliminaryCopyright.passed||Number(preliminaryCopyright.maxSimilarity||0)>=0.42){
    ...
    const fallbackDerived=buildFeedFirstDerivedOutputs({topicTitle,pkg:fallbackPkg,rendered:fallbackRendered,factLedger,evidenceItems,trendMeta,sourceKeyword,requestedTopTitle,identityMode,isFullTier,publishableFacts,contentTier});
    const fallbackEditorialTop=fallbackDerived.editorialTop;
    const fallbackHeadline=fallbackDerived.feedHeadline;
    const fallbackFeed=fallbackDerived.feedTitle;
    const fallbackDetail=fallbackDerived.detailTitle;
    const fallbackCard=fallbackDerived.card;
    const fallbackClaims=fallbackDerived.claimMap;
    const fallbackQa=fallbackDerived.qa;
    const fallbackInstagram=fallbackDerived.instagramCards;
    ...
    preliminaryCopyright=assessCopyrightRisk(draft,copyrightSources,{scope:'feed'});
    preliminaryGrounding=assessGrounding(draft,factLedger,{scope:'feed'});
    aiStatus='verified_fallback';
    aiError=aiError||'COPYRIGHT_REWRITE_FALLBACK';
  }

  let finalAccuracyValidation=validateGeneratedPackageAccuracy(pkg,factLedger,{scope:'feed'});
  if(!finalAccuracyValidation.passed){
    ...
    const strictAccuracy=validateGeneratedPackageAccuracy(strictPkg,factLedger,{scope:'feed'});
    if(strictAccuracy.passed&&Array.isArray(strictPkg?.sections)&&strictPkg.sections.length){
      ...
      const strictDerived=buildFeedFirstDerivedOutputs({topicTitle,pkg:strictPkg,rendered:strictRendered,factLedger,evidenceItems,trendMeta,sourceKeyword,requestedTopTitle,identityMode,isFullTier,publishableFacts,contentTier});
      editorialTop=strictDerived.editorialTop;
      feedHeadline=strictDerived.feedHeadline;
      normalizedFeedTitle=strictDerived.feedTitle;
      normalizedDetailTitle=strictDerived.detailTitle;
      normalizedCard=strictDerived.card;
      claimMap=strictDerived.claimMap;
      qa=strictDerived.qa;
      instagramCards=strictDerived.instagramCards;
  ...
  const feedPublicCharacterCount=feedDetailLength(draft);
  const feedBodyCharacterCount=String(rendered.blog||'').length;
  const feedLengthBreakdown={
    publicText:feedPublicCharacterCount,
    body:feedBodyCharacterCount,
    minimum:FEED_DETAIL_MIN_CHARS,
    target:FEED_DETAIL_TARGET_CHARS,
    countingBasis:'title-summary-points-headings-body-qa-visible-whitespace',
  };
...
export function buildFactBasedStageCandidate(candidate={}, keyword='', trendMeta={}, preferredImageMeta=null, reason='') {
  ...
  const repairAccuracyValidation=validateGeneratedPackageAccuracy(pkg,ledger,{scope:'feed'});
  ...
  const derived=buildFeedFirstDerivedOutputs({
    topicTitle:resolvedKeyword,
    pkg,
    rendered,
    factLedger:ledger,
    evidenceItems:evidence,
    trendMeta,
    sourceKeyword:resolvedKeyword,
    requestedTopTitle:resolvedKeyword,
    identityMode:identityOnly,
    isFullTier:String(trendMeta.contentTier||candidate.contentTier||'standard')==='full',
    publishableFacts:selectedFacts,
    contentTier:trendMeta.contentTier||candidate.contentTier||'standard',
  });
  const editorialTop=derived.editorialTop;
  const headline=derived.feedHeadline;
  const topKeyword=editorialTop.topKeyword||resolvedKeyword;
  const feedTitle=derived.feedTitle;
  const detailTitle=derived.detailTitle;
  const builtCard=derived.card;
  const factSummary=facts.slice(0,4).map(row=>row.text).join(' ').replace(/\s+/g,' ').trim();
  let summary=String(builtCard.summary||factSummary||'').trim();
  if(summary.length<25){
    for(const fact of facts){
      const candidateSummary=`${summary} ${fact.text}`.replace(/\s+/g,' ').trim();
      if(candidateSummary.length>summary.length)summary=candidateSummary;
      if(summary.length>=25)break;
    }
  }
  const safeSummary=summary.slice(0,180);
  ...
  const base={
    ...
    blog:rendered.blog,claimMap:derived.claimMap,card,qa:derived.qa,contentGrade:grade,contentScore:Math.max(Number(candidate.contentScore||0),Number(gradeInfo.contentScore||0)),contentGradeInfo:gradeInfo,
    candidateType:identityOnly?'entity_profile':'event',feedDetailLengthPolicy:'v8056-public-text-min1000-target5000-feed-first',topPreviewLengthPolicy:'v8045-max-1000',causeStatus:identityOnly?'identified':'confirmed',identityMode:identityOnly,contentMode:'detailed',contentType:'detailed',hasContent:true,hasNews:Array.isArray(candidate.relatedNews)&&candidate.relatedNews.length>0,
  ...
  base.instagramCards=ensurePromoCard(derived.instagramCards,base);
  base.feedPublicCharacterCount=feedDetailLength(base);
  base.feedBodyCharacterCount=String(base.blog||'').length;
  base.feedLengthBreakdown={publicText:base.feedPublicCharacterCount,body:base.feedBodyCharacterCount,countingBasis:'title-summary-points-headings-body-qa-visible-whitespace'};
  base.contentPipeline=['top25_ranked_generation_pool','successful_top20_publication','independent_keyword_search','feed_body_first','feed_derived_summary','feed_derived_title','feed_derived_auxiliary'];
...
function upgradeStoredStageContent(stored={}, keyword='', trendMeta={}, preferredImageMeta=null, reason='') {
  ...
  return {
    ...
    contentVersion:CONTENT_VERSION,feedDetailLengthPolicy:'v8056-public-text-min1000-target5000-feed-first',topPreviewLengthPolicy:'v8045-max-1000',status:'published',visibility:'published',reviewRequired:false,
...
export async function getCachedContent(slug, keyword, preferredImage = null, trendMeta = {}, options = {}) {
  ...
  try {
    ...
    // v8.0.49: 최종 공개 검증 전에도 실행별 원본을 먼저 남깁니다.
