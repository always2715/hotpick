...
import { buildResearchEventQueries, mergeEvidenceItems, mergeResearchLedgers } from './researchEvidenceMerge.js';
...
export const CONTENT_VERSION = 139;
...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.55 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.55 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
...
export async function researchTopic(keyword, context={}) {
  ...
  const sourceTitleHints=unique(Array.isArray(context.sourceTitleHints)?context.sourceTitleHints:[]).slice(0,10);
  const eventQueries=buildResearchEventQueries(topicTitle,sourceTitleHints,context.researchTopicHint||'',6);
  const priorCheckpoint=context.priorResearchCheckpoint&&typeof context.priorResearchCheckpoint==='object'?context.priorResearchCheckpoint:null;
  const priorEvidence=mergeEvidenceItems(
    Array.isArray(priorCheckpoint?.evidenceSources)?priorCheckpoint.evidenceSources:Array.isArray(priorCheckpoint?.sourceItems)?priorCheckpoint.sourceItems:[],
    Array.isArray(context.evidenceSources)?context.evidenceSources:[],
    24,
  );
  const independentContext={keyword:topicTitle,topKeyword:topicTitle,researchKeyword:topicTitle,researchAttempt,identityHints,sourceTitleHints,eventQueries,evidenceSources:priorEvidence};
  const cacheKey=`v855:a${researchAttempt}:${identityHints.join('|').toLowerCase().replace(/\s+/g,' ').trim()}:${eventQueries.join('|').toLowerCase().replace(/\s+/g,' ').trim()}`;
  ...
  const baseQueries=unique([eventQueries[0]||topicTitle,topicTitle,...eventQueries,...identityQueries,...(Array.isArray(plan.queries)?plan.queries:[])]).slice(0,8);
  ...
  if(researchAttempt>1){
    const expanded=[
      ...eventQueries.flatMap(query=>[query,`${query} 공식`,`${query} 일정`]),
  ...
  if(priorCheckpoint){
    bundle.items=mergeEvidenceItems(bundle.items||bundle.evidenceSources||[],priorEvidence,24);
    bundle.evidenceSources=bundle.items;
    bundle.factLedger=mergeResearchLedgers(bundle.factLedger,priorCheckpoint.factLedger||{},topicTitle);
    bundle.priorResearchMerged=true;
    bundle.priorResearchFactCount=Array.isArray(priorCheckpoint?.factLedger?.facts)?priorCheckpoint.factLedger.facts.length:0;
  }
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.55; +https://stellate.co.kr)',
...
- 모든 출력은 존댓말로 작성하세요.`;
  ...
  // v8.0.49: TOP 단건 생성은 fastRefresh라 일반 품질 수정 단계를 건너뛰므로,
...
export function buildFactBasedStageCandidate(candidate={}, keyword='', trendMeta={}, preferredImageMeta=null, reason='') {
  ...
  if(summary.length<25){
    for(const fact of facts){
      const candidate=`${summary} ${fact.text}`.replace(/\s+/g,' ').trim();
