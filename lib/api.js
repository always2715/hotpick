...
export const CONTENT_VERSION = 135;
...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.45 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.45 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.45; +https://stellate.co.kr)',
...
export function buildFactBasedStageCandidate(candidate={}, keyword='', trendMeta={}, preferredImageMeta=null, reason='') {
  ...
  const base={
    ...
    candidateType:identityOnly?'entity_profile':'event',feedDetailLengthPolicy:'v8045-min1000-target5000',topPreviewLengthPolicy:'v8045-max-1000',causeStatus:identityOnly?'identified':'confirmed',identityMode:identityOnly,contentMode:'detailed',contentType:'detailed',hasContent:true,hasNews:Array.isArray(candidate.relatedNews)&&candidate.relatedNews.length>0,
...
function upgradeStoredStageContent(stored={}, keyword='', trendMeta={}, preferredImageMeta=null, reason='') {
  ...
  return {
    ...
    contentVersion:CONTENT_VERSION,feedDetailLengthPolicy:'v8045-min1000-target5000',topPreviewLengthPolicy:'v8045-max-1000',status:'published',visibility:'published',reviewRequired:false,
