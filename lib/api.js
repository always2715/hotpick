...
export const CONTENT_VERSION = 131;
...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.35 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.35 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.35; +https://stellate.co.kr)',
...
- 본문은 공백 포함 ${lengthRange.min.toLocaleString()}~${lengthRange.max.toLocaleString()}자를 목표로 하되, 확인된 사실량보다 분량을 우선하지 마세요. 섹션은 3~5개로 작성합니다. 기본정보 → 지금 주목받는 이유 → 핵심 맥락·수치·일정 → 이용자가 알아둘 점 → STELLATE 인사이트 순서에서 근거가 있는 구역만 선택하세요.
