...
export const CONTENT_VERSION = 127;
...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.31 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.31 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
...
export async function researchTopic(keyword, context={}) {
  // TOP20 후보 수집 과정의 기사·사건 맥락과 상세 조사를 완전히 분리합니다.
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.31; +https://stellate.co.kr)',
...
export async function generateContent(keyword, newsBundle, existingContent = null, preferredImage = null, trendMeta = {}) {
  ...
  // TOP20 단계에서 확정한 키워드는 상세 조사에서 다시 기사 제목이나 사건명으로 변형하지 않습니다.
  ...
  const keywordResolution={ok:Boolean(requestedTopKeyword),keyword:requestedTopKeyword,source:'top20_fixed_keyword',candidates:[requestedTopKeyword].filter(Boolean)};
...
export async function getCachedContent(slug, keyword, preferredImage = null, trendMeta = {}, options = {}) {
  ...
  const saveRequestedStage=content=>savePublicationStage({...content,slug,publicationStageId:requestedStageId},{stageId:requestedStageId});
  ...
  if (cached && !force && !checkForUpdates) {
    ...
    if (Date.now() - cached.timestamp < ttl && cached.data?.contentVersion === CONTENT_VERSION && validForFlow) {
      return stageOnly?saveRequestedStage({...cached.data,stageCacheReused:true}):cached.data;
    }
  }
  ...
  if (stored && !force && !checkForUpdates) {
    ...
    if (stored.contentVersion === CONTENT_VERSION && validForFlow && age >= 0 && age < CACHE_TTL) {
      ...
      return stageOnly?saveRequestedStage({...stored,stageCacheReused:true}):stored;
    }
  }
...
async function buildTrendSnapshot({ force=false, dryRun=false, persist=false, onStage=null }={}) {
  ...
  let trends=normalizeCandidateRows(generated?.trends||[]).slice(0,PUBLIC_TOP_COUNT);
  ...
  const warnings=[];
  if(trends.length<PUBLIC_TOP_COUNT)warnings.push(`TOP 20 생성을 위한 조사 후보가 부족합니다. 현재 ${trends.length}개입니다.`);
  ...
  trends=await attachUnsplashImages(trends.slice(0,PUBLIC_TOP_COUNT));
