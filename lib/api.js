...
import { saveContent, saveReviewDraft, getContent, saveTrends, getCachedTrends, getPreviousRanks, recordTokenUsage, getTokenUsage, setContentStatus, CONTENT_STATUS, getTrendsUpdatedAt, acquireLock, releaseLock, updateContentFields, updateTrendFields, getImageCache, setImageCache, getExternalCache, setExternalCache, saveTrendCandidateReport, getTrendRules, savePublicationStage } from './kv';
import { generateTop30, isValidKeyword, cleanKeyword, classifyKeywordWithAI, extractVisualKeyword, buildNeutralTopTitleParts } from './trends';
...
import { sanitizeVisualQuery, buildDeterministicImageQuery, scoreUnsplashCandidate, imageCandidateId } from './images';
...
import { isAutomaticPublicationReady, fixedTop30ContentRejectionReasons, isFixedKeywordFeedReady } from './publicationPolicy';
...
import { validateGeneratedPackageAccuracy, ledgerAccuracyReport, accurateFacts, isGenericFactText } from './contentAccuracy.js';
...
export const CONTENT_VERSION = 126;
...
function buildEditorialHeadline(keyword, source = '', summary = '') {
  ...
  return `${topic}, 확인된 핵심 변화`.slice(0, 32);
}
...
function normalizeFeedTitle(keyword, value, fallbackSummary = '', sourceTitle = '', sourceTitles = []) {
  ...
  if (allSources.some(source => title === source || headlineSimilarity(title, source) >= 0.88)) {
    title = `${normalizedKeyword}, 핵심 흐름과 달라진 점`.slice(0, 32);
  }
  return title;
}
...
function normalizeDetailTitle(keyword, value, feedTitle = '', sourceTitles = []) {
  ...
  if (invalid) {
    ...
    title = feed ? `${feed}, 배경과 핵심 쟁점 정리` : `${topic}, 현재 상황과 핵심 쟁점 정리`;
  }
...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.30 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.30 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
...
export async function researchTopic(keyword, context={}) {
  // TOP30 후보 수집 과정의 기사·사건 맥락과 상세 조사를 완전히 분리합니다.
  const topicTitle=cleanText(context.researchKeyword||keyword).slice(0,60);
  const researchAttempt=Math.max(1,Math.min(5,Number(context.researchAttempt||1)));
  const independentContext={keyword:topicTitle,topKeyword:topicTitle,researchKeyword:topicTitle,researchAttempt};
  const cacheKey=`v829:a${researchAttempt}:${topicTitle.toLowerCase().replace(/\s+/g,' ').trim()}`;
  ...
  if(researchAttempt>1){
    const expanded=[
      `${topicTitle} 최신`,`${topicTitle} 공식`,`${topicTitle} 주요 활동`,`${topicTitle} 일정`,
      ...(Array.isArray(plan.queries)?plan.queries:[]),topicTitle,
    ];
    const profileExpanded=[`${topicTitle} 기본정보`,`${topicTitle} 프로필`,`${topicTitle} 공식`,...(Array.isArray(plan.profileQueries)?plan.profileQueries:[])];
    const unique=rows=>[...new Set(rows.map(cleanText).filter(Boolean))];
    plan={...plan,queries:unique(expanded).slice(0,6),profileQueries:unique(profileExpanded).slice(0,4),researchAttempt,expandedResearch:true};
  }
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.30; +https://stellate.co.kr)',
...
// ─── Unsplash 이미지: 사건 관련성·중복 방지·출처 정보 ─────────────
const UNSPLASH_UTM = 'utm_source=stellate&utm_medium=referral';
const IMAGE_SELECTOR_VERSION = 'v4';
...
function normalizeImageMeta(value, extras = {}) {
  ...
  const imageUrl = value.imageUrl || value.url || value.urls?.regular || null;
  if (!imageUrl) return null;
  return {
    id: value.id || imageUrl,
    source: value.source || 'unsplash', imageUrl, thumbUrl: value.thumbUrl || value.urls?.small || imageUrl,
    ...
    unsplashPhotoUrl: value.unsplashPhotoUrl || withUnsplashUtm(value.links?.html) || null,
    ...
    tags: Array.isArray(value.tags) ? value.tags.map(tag => tag?.title || tag?.name || '').filter(Boolean).slice(0, 12) : [],
    selectorVersion: IMAGE_SELECTOR_VERSION,
  };
}

async function fetchUnsplashCandidates(query, limit = 10) {
  const ACCESS_KEY = process.env.UNSPLASH_ACCESS_KEY;
  const safeQuery = sanitizeVisualQuery(query);
  if (!ACCESS_KEY || !safeQuery) return [];
  const cacheKey = `${IMAGE_SELECTOR_VERSION}:${safeQuery}`;
  const cached = await getImageCache(cacheKey);
  if (Array.isArray(cached?.results)) return cached.results;
  try {
    const count = Math.max(4, Math.min(15, Number(limit || 10)));
    const res = await fetch(
      `https://api.unsplash.com/search/photos?query=${encodeURIComponent(safeQuery)}&per_page=${count}&orientation=landscape&content_filter=high`,
      { headers: { Authorization: `Client-ID ${ACCESS_KEY}`, 'Accept-Version': 'v1' }, signal: externalSignal(8000) }
    );
    if (!res.ok) return [];
    const data = await res.json();
    const results = (Array.isArray(data.results) ? data.results : []).map(photo => ({
      id: photo.id || null,
      urls: { regular: photo.urls?.regular || null, small: photo.urls?.small || null },
      user: { name: photo.user?.name || '', links: { html: photo.user?.links?.html || null } },
      links: { html: photo.links?.html || null, download_location: photo.links?.download_location || null },
      alt_description: photo.alt_description || '', description: photo.description || '',
      tags: Array.isArray(photo.tags) ? photo.tags.map(tag => ({ title: tag?.title || tag?.name || '' })).filter(tag => tag.title) : [],
    })).filter(photo => photo.urls.regular);
    await setImageCache(cacheKey, { results, query:safeQuery, selectorVersion:IMAGE_SELECTOR_VERSION }, 60 * 60 * 24 * 7);
    return results;
  } catch { return []; }
}

function chooseUnsplashCandidate(candidates, spec, usedIds = new Set()) {
  const ranked = (Array.isArray(candidates) ? candidates : []).map((photo, rank) => {
    const evaluation = scoreUnsplashCandidate(photo, spec.query, spec.avoid || [], rank);
    return { photo, ...evaluation, id:imageCandidateId(photo) };
  }).filter(row => row.id && !usedIds.has(row.id) && row.score >= Number(spec.minScore || 46))
    .sort((a,b) => b.score - a.score);
  const selected = ranked[0];
  if (!selected) return null;
  return normalizeImageMeta(selected.photo, {
    query:spec.query,
    imageConfidence:Math.max(0, Math.min(100, selected.score)),
    imageReason:spec.reason || '사건별 이미지 검색',
  });
}
...
export async function fetchUnsplashImage(query) {
  ...
  const spec = { query:safeQuery, avoid:[], reason:'직접 이미지 검색', minScore:45 };
  return chooseUnsplashCandidate(await fetchUnsplashCandidates(safeQuery, 10), spec, new Set());
}

function trendImageContext(item = {}) {
  const articleTitles = (Array.isArray(item.dominantArticles) ? item.dominantArticles : [])
    .map(row => row?.title || '').filter(Boolean).slice(0, 3).join(' / ');
  return `${item.topKeyword || item.keyword || ''} ${item.topTopic || ''} ${articleTitles}`.trim();
}

async function createBatchVisualQueries(items, apiKey) {
  if (!apiKey || !Array.isArray(items) || !items.length || claudeAuthUnavailable) return new Map();
  const rows = items.map((item, index) => ({
    id:String(index), category:item.category || 'general', title:item.topTitle || item.displayTitle || item.keyword || '',
    context:trendImageContext(item).slice(0, 300),
  }));
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      method:'POST',
      headers:{ 'Content-Type':'application/json', 'x-api-key':apiKey, 'anthropic-version':'2023-06-01' },
      signal:externalSignal(12000),
      body:JSON.stringify({
        model:'claude-haiku-4-5-20251001', max_tokens:1400,
        messages:[{role:'user',content:`다음 TOP 이슈 각각에 대해 Unsplash 검색용 영어 구문을 만드세요.\n\n규칙:\n- 사건을 직접 보여 주는 물체·장소·현장을 3~6개 영어 단어로 표현\n- news, current trends, breaking news, protest, demonstration 같은 범용 단어 금지\n- 인물 얼굴·브랜드 로고·기사 화면을 찾지 말고 사건을 상징하는 실제 장면을 선택\n- 원전 이슈는 원전 시설, 서비스 장애는 서버·데이터센터, 정책은 기관 건물처럼 구체화\n- 관련 장면을 안전하게 특정할 수 없으면 query를 빈 문자열로 반환\n- avoid에는 잘못 매칭되기 쉬운 영어 단어를 최대 4개 입력\n\nJSON 배열만 반환:\n[{"id":"0","query":"...","avoid":["..."],"reason":"..."}]\n\n대상:\n${JSON.stringify(rows)}`}],
      }),
    });
    if (!response.ok) {
      if (response.status === 401 || response.status === 403) claudeAuthUnavailable = true;
      return new Map();
    }
    const data = await response.json();
    const parsed = JSON.parse(String(data.content?.[0]?.text || '').replace(/```json|```/g,'').trim());
    return new Map((Array.isArray(parsed) ? parsed : []).map(row => [String(row.id), {
      query:sanitizeVisualQuery(row.query), avoid:Array.isArray(row.avoid) ? row.avoid.slice(0,4) : [],
      reason:String(row.reason || 'AI 사건 이미지 해석').slice(0,80), source:'ai', minScore:46,
    }]).filter(([,spec]) => spec.query));
  } catch { return new Map(); }
}

function selectRelevantVideoImage(videos=[], keyword='') {
  const keywordTokens=headlineTokens(keyword).filter(token=>token.length>1);
  const rows=(Array.isArray(videos)?videos:[]).filter(video=>video?.thumbnail&&video?.title);
  const matched=rows.find(video=>{
    const titleTokens=headlineTokens(video.title);
    return keywordTokens.length&&keywordTokens.some(token=>titleTokens.some(value=>value.includes(token)||token.includes(value)));
  });
  if(!matched)return null;
  return {
    id:`youtube-${matched.id||matched.thumbnail}`,
    source:'youtube',
    imageUrl:matched.thumbnail,
    thumbUrl:matched.thumbnail,
    query:keyword,
    imageConfidence:82,
    imageReason:'주제와 직접 연결된 최근 YouTube 영상 썸네일',
    sourceUrl:matched.url||`https://www.youtube.com/watch?v=${matched.id}`,
    channel:matched.channel||'YouTube',
  };
}

function selectRelevantSourceImage(items=[], keyword='', shortTitle='') {
  const topicTokens=headlineTokens(`${keyword} ${shortTitle}`).filter(token=>token.length>1);
  const ranked=(Array.isArray(items)?items:[])
    .filter(item=>/^https?:\/\//i.test(String(item?.imageUrl||item?.thumbnail||'')))
    .filter(item=>['official','authorized','trusted_news'].includes(String(item?.sourceType||'')))
    .map(item=>{
      const titleTokens=headlineTokens(`${item.title||''} ${item.description||''}`);
      const overlap=topicTokens.filter(token=>titleTokens.some(value=>value.includes(token)||token.includes(value))).length;
      const sourceScore=item.sourceType==='official'?30:item.sourceType==='authorized'?24:16;
      return {item,score:sourceScore+overlap*12};
    })
    .filter(row=>row.score>=28)
    .sort((a,b)=>b.score-a.score);
  const selected=ranked[0]?.item;
  if(!selected)return null;
  const imageUrl=selected.imageUrl||selected.thumbnail;
  return {
    id:`source-${selected.domain||selected.source||imageUrl}`,
    source:selected.sourceType==='official'?'official_source':'trusted_source',
    imageUrl,thumbUrl:imageUrl,query:`${keyword} ${shortTitle}`.trim(),
    imageConfidence:selected.sourceType==='official'?92:84,
    imageReason:selected.sourceType==='official'?'공식 자료 대표 이미지':'주제와 직접 연결된 신뢰 출처 대표 이미지',
    sourceUrl:selected.link||selected.canonicalUrl||null,
    provider:selected.source||selected.domain||'외부 출처',
  };
}


// 상세 대표 이미지: 콘텐츠가 만든 시각 키워드를 우선하되 관련도 미달이면 사진을 강제하지 않는다.
async function resolveCoverImage(keyword, summary, categoryKey, apiKey, providedVisualQuery = '') {
  const selectionSuffix=String(`${keyword||''}-${summary||''}`).toLowerCase().replace(/[^0-9a-z가-힣]+/g,'-').replace(/^-|-$/g,'').slice(0,120);
  const selectionKey=`selected-v8018:${selectionSuffix}`;
  const cachedSelection=await getImageCache(selectionKey)||await getImageCache(`selected-v8017:${selectionSuffix}`);
  if(cachedSelection?.imageUrl){
    const normalized=normalizeImageMeta(cachedSelection);
    await setImageCache(selectionKey,normalized,60*60*24*7);
    return normalized;
  }
  const provided = sanitizeVisualQuery(providedVisualQuery);
  const extracted = provided ? '' : sanitizeVisualQuery(await extractVisualKeyword(keyword, summary, apiKey));
  const deterministic = buildDeterministicImageQuery({ keyword, topic:summary, category:categoryKey, context:summary });
  const categoryFallback={
    entertainment:'concert stage entertainment',sports:'sports stadium competition',economy:'financial market business',tech:'technology data center',ai:'artificial intelligence technology',
    life:'city daily life',politics:'government institutional building',travel:'travel destination landscape',general:'city public information',
  }[categoryKey]||'city public information';
  const spec = provided
    ? { query:provided, avoid:deterministic?.avoid || [], reason:'검증 콘텐츠 시각 키워드', minScore:45 }
    : extracted
      ? { query:extracted, avoid:deterministic?.avoid || [], reason:'AI 시각 키워드', minScore:47 }
      : deterministic||{query:categoryFallback,avoid:[],reason:'카테고리 안전 대체 이미지',minScore:38};
  if (!spec?.query) return null;
  const selected=chooseUnsplashCandidate(await fetchUnsplashCandidates(spec.query, 12), spec, new Set());
  if(selected?.imageUrl)await setImageCache(selectionKey,selected,60*60*24*7);
  return selected;
}

// TOP 이미지는 한 번의 AI 질의로 사건별 검색어를 만들고, 후보 다건 비교·중복 제거 후 적용한다.
async function attachUnsplashImages(trends) {
  const list = Array.isArray(trends) ? trends : [];
  if (!process.env.UNSPLASH_ACCESS_KEY || list.length === 0) {
    // 이미지 API가 비활성 또는 일시 실패해도 Upstash/직전 정상 이미지 값을 지우지 않습니다.
    return list.map(item => {
      const cachedMeta=item.imageMeta||null;
      const cachedUrl=item.thumbnail||cachedMeta?.thumbUrl||cachedMeta?.imageUrl||null;
      return {...item,thumbnail:cachedUrl,thumbnailSource:item.thumbnailSource||cachedMeta?.source||null,imageMeta:cachedMeta};
    });
  }

  const aiSpecs = await createBatchVisualQueries(list, process.env.ANTHROPIC_API_KEY);
  const specs = list.map((item, index) => {
    const deterministic = buildDeterministicImageQuery({
      keyword:item.topKeyword || item.keyword || item.displayTitle,
      topic:item.topTopic || '', category:item.category || 'general', context:trendImageContext(item),
    });
    const aiSpec = aiSpecs.get(String(index));
    if (!aiSpec) return deterministic;
    return { ...aiSpec, avoid:[...new Set([...(aiSpec.avoid || []), ...(deterministic?.avoid || [])])], minScore:46 };
  });
  ...
  for (let start=0; start<list.length; start+=batchSize) {
    const batchItems = list.slice(start,start+batchSize);
    const batchSpecs = specs.slice(start,start+batchSize);
    const candidateLists = await Promise.all(batchSpecs.map(spec => spec?.query ? fetchUnsplashCandidates(spec.query, 12) : Promise.resolve([])));
    for (let offset=0; offset<batchItems.length; offset++) {
      const item = batchItems[offset];
      const spec = batchSpecs[offset];
      const imageMeta = spec ? chooseUnsplashCandidate(candidateLists[offset], spec, usedIds) : null;
      if (imageMeta?.id) usedIds.add(imageMeta.id);
      output.push({
        ...item,
        thumbnail:imageMeta?.thumbUrl || imageMeta?.imageUrl || null,
        thumbnailSource:imageMeta ? 'Unsplash' : null,
        imageMeta,
        imageQuery:spec?.query || null,
        imageConfidence:imageMeta?.imageConfidence || 0,
      });
    }
  }
  return output;
...
export function assessContentQuality(content) {
  ...
  const minimumLength=Math.max(120,gradeRange.min-80);
  const maximumLength=grade==='A'?2400:grade==='B'?1400:grade==='C'?900:900;
  if(length<minimumLength){score-=35;reasons.push(`등급별 본문 권장 최소 ${minimumLength.toLocaleString()}자 미만`);}
  else if(length>maximumLength){score-=12;reasons.push('등급별 권장 분량을 초과');}
...
function renderBlogPackage(pkg = {}, onlineTrend = null, context={}) {
  const rawSections=Array.isArray(pkg.sections)?pkg.sections:[];
  const basicRaw=rawSections.find(section=>/기본|배경|알아보기|무엇/i.test(cleanText(section?.heading)))||rawSections[0]||{};
  const issuesRaw=rawSections.find(section=>/이슈|변화|달라|핵심|현재|지금/i.test(cleanText(section?.heading)))||rawSections[1]||{};
  const insightRaw=rawSections.find(section=>/STELLATE\s*인사이트|종합\s*(?:정리|해석)|핵심\s*해석/i.test(cleanText(section?.heading)))||rawSections[2]||{};
  const basicProfileText=(Array.isArray(basicRaw?.paragraphs)?basicRaw.paragraphs:[]).map(row=>typeof row==='string'?row:row?.text||'').join(' ');
  const basic=normalizedEditorialSection(basicRaw,naturalFeedHeading('basic',basicRaw?.heading,{keyword:context.keyword||'',eventTitle:context.eventTitle||pkg.shortTitle||'',profileText:basicProfileText}));
  const issues=normalizedEditorialSection(issuesRaw,naturalFeedHeading('issues',issuesRaw?.heading,{keyword:context.keyword||'',eventTitle:context.eventTitle||pkg.shortTitle||''}));
  const insight=normalizedEditorialSection(insightRaw,naturalFeedHeading('insight',insightRaw?.heading,{keyword:context.keyword||'',eventTitle:context.eventTitle||pkg.shortTitle||''}));
  // 온라인 반응은 내부 참고 데이터로 유지하되 피드 본문과 인사이트의 사실 근거에는 넣지 않습니다.
  const sections=[basic,issues,insight];
  const chunks=[];const factualChunks=[];const claimMap=[];
  // intro는 요약 카드·검증용 메타데이터로 유지하되, 공개 피드 본문은
  // 기본정보 섹션부터 시작해 사용자가 확정한 화면 구조와 일치시킵니다.
...
export async function generateContent(keyword, newsBundle, existingContent = null, preferredImage = null, trendMeta = {}) {
  ...
  // TOP30 단계에서 확정한 키워드는 상세 조사에서 다시 기사 제목이나 사건명으로 변형하지 않습니다.
  const fixedTop={topKeyword:requestedTopKeyword,topTopic:'',topTitle:'',displayTitle:requestedTopKeyword};
  ...
  const keywordResolution={ok:Boolean(requestedTopKeyword),keyword:requestedTopKeyword,source:'top30_fixed_keyword',candidates:[requestedTopKeyword].filter(Boolean)};
  topicTitle=String(requestedTopKeyword||sourceKeyword).trim();
  ...
  if (contentTier === 'brief') {
    const briefImageMeta=preferredImageMeta||selectRelevantVideoImage(relatedVideos,topicTitle)||selectRelevantSourceImage(evidenceItems,topicTitle,'관련 관심 증가')||await resolveCoverImage(topicTitle,'관련 관심 흐름',categoryKey,claudeAuthUnavailable?null:usableClaudeKey,'');
  ...
  const fixedKeywordFlow=trendMeta.fixedTop30Flow===true;
...
[엄격한 규칙]
- 반드시 Fact ID(${validFactIds})로 확인되는 사실만 사용하세요. PF로 시작하는 기본정보 Fact는 첫 번째 섹션에만, F로 시작하는 현재 이슈 Fact는 두 번째 섹션과 인사이트에 사용하세요. 기억, 일반 상식, 추측, 전망을 추가하지 마세요.
...
- 본문은 공백 포함 ${lengthRange.min.toLocaleString()}~${lengthRange.max.toLocaleString()}자이며, 섹션은 기본정보 → 현재 이슈 → STELLATE 인사이트 순서로 정확히 3개 작성하세요.
- 첫 번째와 두 번째 소제목은 내용을 읽고 자연스럽고 구체적으로 직접 만드세요. 첫 번째는 ‘박서진은 어떤 가수인가’, ‘갤럭시 Z 폴드는 어떤 제품인가’처럼 키워드와 실제 대상 유형이 드러나야 합니다. 두 번째는 ‘전국투어 공연 예매 일정 공개’처럼 현재 단일 사건이 드러나야 합니다. ‘키워드 기본정보’, ‘이슈사항’, ‘주요 동향’ 같은 고정·포괄 제목은 쓰지 마세요.
...
- 두 번째 섹션은 발행 시각이 내부 수집 기준에 포함되는 현재 이슈 Fact만 정리하고, 기본정보 출처에 있는 오래된 사건은 섞지 마세요.
...
- summary·why·points는 JSON 형식 유지를 위한 초안입니다. 실제 요약정보 카드는 전체 피드 작성 후 시스템이 피드 내용만 다시 요약합니다. 요약정보는 현재 이슈를 설명하는 1~2개 문단과 실제 주요 내용 3~5개로 구성하며, 피드에 없는 일반 문구를 추가하지 않습니다.
${tierRules}
...
- 모든 출력은 존댓말로 작성하세요.`;
  ...
  const guard=evaluateCostGuard({usage:todayUsage,rank:trendRank,grade:rankingGrade,contentTier,prompt,requestedOutput:isFullTier?4800:3800});
  ...
  let editorialTop=identityMode?{topKeyword:topicTitle,topTopic:pkg.shortTitle||'기본정보와 주요 특징',topTitle:pkg.topTitle||`${topicTitle} · ${pkg.shortTitle||'기본정보와 주요 특징'}`,displayTitle:topicTitle,shortTitle:pkg.shortTitle||'기본정보와 주요 특징',titleStatus:'ready',titleReady:true,titleSource:'keyword_identity_fallback',titleValidationReasons:[],keywordSource:'fixed_top30_keyword'}:derivePostResearchTitle(topicTitle,pkg,factLedger,evidenceItems.map(x=>x.title),{detailContent:rendered.factualBlog,rawKeyword:trendMeta.rawKeyword||sourceKeyword,displayTitle:requestedTopTitle,fixedKeyword:true});
  ...
  let normalizedDetailTitle=normalizedFeedTitle;
  ...
  let preliminaryQuality=assessContentQuality({...draft,sourceItems:evidenceItems});
  ...
  if(aiStatus==='claude'&&guard.allowRevision&&trendMeta.fastRefresh!==true&&(preliminaryQuality.score<82||!preliminaryCopyright.passed||preliminaryGrounding.score<90)){
    ...
    try{
      ...
      if(revisedPkg){
        ...
        const revisedDetail=revisedFeed;
        ...
        const candidateQuality=assessContentQuality({...candidate,sourceItems:evidenceItems});
  ...
  if(!preliminaryCopyright.passed||Number(preliminaryCopyright.maxSimilarity||0)>=0.42){
    ...
    const fallbackEditorialTop=identityMode?{topKeyword:topicTitle,topTopic:fallbackPkg.shortTitle||'기본정보와 주요 특징',topTitle:fallbackPkg.topTitle||`${topicTitle} · ${fallbackPkg.shortTitle||'기본정보와 주요 특징'}`,displayTitle:topicTitle,shortTitle:fallbackPkg.shortTitle||'기본정보와 주요 특징',titleStatus:'ready',titleReady:true,titleSource:'keyword_identity_fallback',titleValidationReasons:[],keywordSource:'fixed_top30_keyword'}:derivePostResearchTitle(topicTitle,fallbackPkg,factLedger,evidenceItems.map(x=>x.title),{detailContent:fallbackRendered.factualBlog,rawKeyword:trendMeta.rawKeyword||sourceKeyword,displayTitle:requestedTopTitle,fixedKeyword:true});
    ...
    const fallbackDetail=fallbackFeed;
    ...
    preliminaryQuality=assessContentQuality({...draft,sourceItems:evidenceItems});
  ...
  const finalAccuracyValidation=validateGeneratedPackageAccuracy(pkg,factLedger);
  if(!finalAccuracyValidation.passed){
    const error=new Error(`생성 문안이 Fact Ledger와 정확히 일치하지 않습니다: ${finalAccuracyValidation.problems.join(' / ')}`);
    error.code='STRICT_CONTENT_ACCURACY_FAILED';
    error.details=finalAccuracyValidation;
    throw error;
  }
  ...
  const videoImageMeta=selectRelevantVideoImage(videos,`${editorialTop.topKeyword} ${editorialTop.shortTitle||editorialTop.topTopic||''}`);
  const sourceImageMeta=selectRelevantSourceImage(evidenceItems,editorialTop.topKeyword,editorialTop.shortTitle||editorialTop.topTopic||'');
  const imageMeta=preferredImageMeta||videoImageMeta||sourceImageMeta||await resolveCoverImage(editorialTop.topTitle||editorialTop.topKeyword||topicTitle,normalizedCard.summary,categoryKey,claudeAuthUnavailable?null:usableClaudeKey,visualQuery);
...
export function buildFactBasedStageCandidate(candidate={}, keyword='', trendMeta={}, preferredImageMeta=null, reason='') {
  ...
  const originalLedger=candidate.factLedger||{version:3,sources:[],facts:[],confirmedFacts:[],uncertainties:[],conflicts:[]};
  const facts=(Array.isArray(originalLedger.facts)?originalLedger.facts:[])
    .map((row,index)=>({
      ...row,
      id:row?.id||`F${index+1}`,
  ...
  const evidence=(directEvidence.length?directEvidence:ledgerEvidence).map((row,index)=>{
    const link=canonicalizeUrl(row?.link||row?.url||row?.canonicalUrl||'');
    const rawType=String(row?.sourceType||'').trim();
    const sourceType=['official','authorized','trusted_news','independent'].includes(rawType)?rawType:(link?'independent':'');
    return {...row,id:row?.id||`E${index+1}`,link,canonicalUrl:link,domain:row?.domain||sourceFromUrl(link,''),sourceType,evidenceUsable:Boolean(link&&sourceType),contentUsable:Boolean(link&&sourceType)};
  }).filter(row=>row.link&&row.sourceType);
  ...
  if(!ledgerReport.passed||selectedFacts.every(row=>isGenericFactText(row.text)))return null;
  ...
  const detailTitle=normalizeDetailTitle(topKeyword,`${feedTitle}, 배경과 핵심 내용 정리`,feedTitle,[]);
  ...
  const base={
    ...
    candidateType:identityOnly?'entity_profile':'event',causeStatus:identityOnly?'identified':'confirmed',identityMode:identityOnly,contentMode:'detailed',contentType:'detailed',hasContent:true,hasNews:Array.isArray(candidate.relatedNews)&&candidate.relatedNews.length>0,
    ...
    image:preferredImageMeta?.imageUrl||candidate.image||candidate.imageMeta?.imageUrl||null,imageMeta:preferredImageMeta||candidate.imageMeta||null,imageSource:(preferredImageMeta||candidate.imageMeta)?.source||candidate.imageSource||null,
...
function upgradeStoredStageContent(stored={}, keyword='', trendMeta={}, preferredImageMeta=null, reason='') {
  ...
  return {
    ...
    image:preferredImageMeta?.imageUrl||stored.image||stored.imageMeta?.imageUrl||null,imageMeta:preferredImageMeta||stored.imageMeta||null,
    contentVersion:CONTENT_VERSION,status:'published',visibility:'published',reviewRequired:false,
...
export async function getCachedContent(slug, keyword, preferredImage = null, trendMeta = {}, options = {}) {
  ...
  const fixedTop30Flow = Boolean(options.fixedTop30Flow);
  ...
  if (cached && !force && !checkForUpdates) {
    ...
    const validForFlow=fixedTop30Flow?reusableFixedStageContent(cached.data,trendMeta.keyword||keyword):contentIsReady(cached.data);
    if (Date.now() - cached.timestamp < ttl && cached.data?.contentVersion === CONTENT_VERSION && validForFlow) {
      return stageOnly?{...cached.data,stageCacheReused:true}:cached.data;
    }
  }
  ...
  if (stored && !force && !checkForUpdates) {
    ...
    const validForFlow=fixedTop30Flow?reusableFixedStageContent(stored,trendMeta.keyword||keyword):contentIsReady(stored);
    if (stored.contentVersion === CONTENT_VERSION && validForFlow && age >= 0 && age < CACHE_TTL) {
      memCache.set(slug, { data: stored, timestamp: Date.now() });
      return stageOnly?{...stored,stageCacheReused:true}:stored;
    }
  }
  ...
  try {
    ...
    if (shouldSkipSourceFetchForBrief(trendMeta)) {
      ...
      try {
        ...
        if (!isTopBriefEligible(trendMeta) && !fixedTop30Flow) throw sourceError;
        sourceFetchWarning = String(sourceError?.message || '출처 조회 실패').slice(0, 240);
        if(stageOnly&&fixedTop30Flow){
          if(contentIsReady(stored)&&Number(stored?.contentVersion||0)===CONTENT_VERSION){
            ...
            if(reusedStage&&isFixedKeywordFeedReady(reusedStage))return savePublicationStage({...reusedStage,slug,top30CompletionPolicy:'fixed_keyword_feed_v15'});
          }
          throw sourceError;
        }
        newsBundle = {
    ...
    if (!force && contentIsReady(stored) && versionOk && sourceSignature && stored.sourceSignature === sourceSignature) {
      const reused = await saveContent({ ...stored, image:preferredImageUrl||stored.image||null, imageMeta:preferredImageMeta||stored.imageMeta||null, lastCheckedAt:new Date().toISOString() });
    ...
    const content = await generateContent(keyword, newsBundle, comparableExisting, preferredImageMeta, {...trendMeta,fixedTop30Flow});
    const candidate = { ...content, slug, ...(sourceFetchWarning ? { sourceFetchWarning } : {}), lastCheckedAt:new Date().toISOString() };

    if(stageOnly){
      const fixedTopReady=fixedTop30Flow&&contentIsReady(candidate)&&isFixedKeywordFeedReady(candidate);
      if(contentIsReady(candidate) && (isAutomaticPublicationReady(candidate)||fixedTopReady)){
        return savePublicationStage({
          ...candidate,
          ...(fixedTop30Flow?{status:'published',visibility:'published',reviewRequired:false,publicationReasons:[],publicationDecision:{...(candidate.publicationDecision||{}),publishable:true,status:'published',visibility:'published',reviewRequired:false,reasons:[]}}:{}),
          top30CompletionPolicy:fixedTop30Flow?'fixed_keyword_feed_v15':candidate.top30CompletionPolicy||null,
        });
      }
      if(fixedTop30Flow){
        const downgradeReasons=fixedTop30ContentRejectionReasons(candidate||{});
        const factualRepair=buildFactBasedStageCandidate(candidate,keyword,trendMeta,preferredImageMeta,downgradeReasons.join(' / '));
        if(factualRepair&&isFixedKeywordFeedReady(factualRepair)){
          return savePublicationStage({...factualRepair,slug,searchQuery:String(trendMeta.keyword||keyword||topicTitle),rawKeyword:trendMeta.rawKeyword||trendMeta.keyword||keyword,eventKey:trendMeta.eventKey||null,category:trendMeta.category||'general',top30CompletionPolicy:'fixed_keyword_feed_v15'});
        }
    ...
    if(stageOnly&&fixedTop30Flow){
      ...
      if(contentIsReady(stored)){
        ...
        if(reusedStage&&isFixedKeywordFeedReady(reusedStage)){
          return savePublicationStage({...reusedStage,slug,top30CompletionPolicy:'fixed_keyword_feed_v15'});
        }
      }
...
async function buildTrendSnapshot({ force=false, dryRun=false, persist=false, onStage=null }={}) {
  ...
  let trends=normalizeCandidateRows(generated?.trends||[]).slice(0,30);
  ...
  const warnings=[];
  if(trends.length<30)warnings.push(`TOP 30 생성을 위한 조사 후보가 부족합니다. 현재 ${trends.length}개입니다.`);
  ...
  trends=await attachUnsplashImages(trends.slice(0,30));
