...
export const CONTENT_VERSION = 130;
...
async function fetchKmaWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(9000),headers:{'User-Agent':'STELLATEBot/8.0.34 (+https://stellate.co.kr)'}});
...
async function fetchOpenMeteoWeatherStructuredEvidence(plan, topicTitle=''){
  ...
  const rows=await mapWithConcurrency(cities,3,async city=>{
    try{
      ...
      const res=await fetch(url,{signal:externalSignal(8000),headers:{'User-Agent':'STELLATEBot/8.0.34 (+https://stellate.co.kr)'}});
      if(!res.ok)return null;
      const data=await res.json();
    ...
    for (let attempt=1; attempt<=2; attempt++) {
      ...
      try {
        const response = await fetch(candidateUrl, {
          ...
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; STELLATEBot/8.0.34; +https://stellate.co.kr)',
...
- 모든 출력은 존댓말로 작성하세요.`;
  ...
  if(aiStatus==='claude'&&guard.allowRevision&&trendMeta.fastRefresh!==true&&(preliminaryQuality.score<82||!preliminaryCopyright.passed||preliminaryGrounding.score<90)){
    ...
    try{
      ...
      if(revisedPkg){
        ...
        // v8.0.34: 품질 점수가 높아도 Fact Ledger 정확성 검사를 통과하지 못한 수정안은 채택하지 않습니다.
  ...
  if(!finalAccuracyValidation.passed){
    // v8.0.34: AI 초안 또는 품질 수정안이 마지막 정확성 검사를 통과하지 못하면
    ...
    if(strictAccuracy.passed&&Array.isArray(strictPkg?.sections)&&strictPkg.sections.length){
      ...
      normalizedDetailTitle=normalizedFeedTitle;
