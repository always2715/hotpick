...
import { containsPublicMetaCopy } from './publicCopy.js';
...
export function isGenericFactText(value='') {
  ...
  if(containsPublicMetaCopy(text))return true;
...
export function validateGeneratedPackageAccuracy(pkg={},ledger={},options={}) {
  const scope=typeof options==='string'?options:String(options?.scope||'all');
  const facts=accurateFacts(ledger,{scope:'all',limit:50,allowSingleTrusted:true});
  const deterministicFactProjection=String(pkg?.accuracyProjection||'').startsWith('fact_ledger_literal_');
  ...
  // v8.0.56: 피드 우선 검증에서는 본문 섹션만 판정합니다. 요약·Q&A·인스타 카드는
  // 검증된 피드에서 이후 파생되므로 보조 출력 오류가 피드 전체를 폐기하지 않습니다.
  if(scope!=='feed'&&pkg.intro)rows.push({kind:'intro',text:cleanText(pkg.intro?.text||pkg.intro),claimIds:pkg.intro?.claimIds||[]});
  if(scope!=='feed')for(const row of Array.isArray(pkg.qa)?pkg.qa:[])rows.push({kind:'qa',text:cleanText(row?.a),claimIds:row?.claimIds||[]});
  if(scope!=='feed')for(const row of Array.isArray(pkg.instagramCards)?pkg.instagramCards:[])rows.push({kind:'instagram',text:cleanText(row?.body),claimIds:row?.claimIds||[]});
  ...
  for(const row of checked){
    ...
    // 결정론적 fallback은 각 문장을 연결 Fact에서 직접 투영하므로 한국어 어미 변환 때문에
    // 토큰 유사도가 낮아지는 경우만 예외 처리합니다. Fact ID·숫자·날짜·추론 검사는 유지합니다.
    if(!deterministicFactProjection&&!semanticSupported(row.text,linked))problems.push(`${row.kind}:연결 사실과 의미 불일치`);
