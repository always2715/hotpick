...
export function isCurrentPublicTop(trend = {}, { limit = PUBLIC_TOP_COUNT } = {}) {
  if (!visibilityAllowsPublicTop(trend)) return false;
  const rank = Number(trend.rank || 0);
  const maxRank = Math.min(PUBLIC_TOP_COUNT, Math.max(1, Number(limit || PUBLIC_TOP_COUNT)));
...
export function isResearchCandidate(trend={}) {
  const rank=Number(trend.rank||0);
  return trend.publicTopPolicy==='research_pending_v3'&&trend.mainVisible===true&&rank>=1&&rank<=PUBLIC_TOP_COUNT;
}
...
export function selectTopContentCandidates(trends = [], { limit = PUBLIC_TOP_COUNT } = {}) {
  const maxRank = Math.min(PUBLIC_TOP_COUNT, Math.max(1, Number(limit || PUBLIC_TOP_COUNT)));
