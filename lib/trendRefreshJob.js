...
async function assertRunCanContinue(runId,phase,{countStep=false}={}) {
  ...
  if(String(run?.engineVersion||'')&&String(run.engineVersion)!=='8.0.33')throw new TrendRefreshError('top20_new_run_required',`이 실행은 ${run.engineVersion} 엔진에서 시작돼 v8.0.33 단일 후보 처리 구조로 재개할 수 없습니다. 기존 작업을 중단하고 새 TOP20 작업을 시작하세요.`,{runId,phase,engineVersion:run.engineVersion});
...
export async function startTrendRefreshRun(runId, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  try {
    ...
    const terminalCodes = new Set(['no_research_candidates','abnormal_top_shrink','trend_refresh_in_progress','trend_lock_unavailable','top20_new_run_required']);
