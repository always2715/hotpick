...
import { assessTrendRunCompatibility, CURRENT_TREND_ENGINE_VERSION } from './trendEnginePolicy';
import { isUnsplashImageUrl, isUnsplashImageMeta } from './images';
...
import { resolveTop30Keyword } from './editorialTitle.js';
import {
  MAX_PREVIOUS_TOP_CARRYOVER,
  MIN_FRESH_PUBLICATION_COUNT,
  carryoverTrendCandidates,
  buildHybridPublicationRows,
} from './partialTopPublication.js';
import { assessResearchBackedFeedSet, assessFeedDuplicateRisk } from './feedSetQuality';
import { ensureThumbnailPoolReady } from './thumbnailPoolService.js';
import { backfillMissingTopThumbnails } from './thumbnailBackfillService.js';
import {
  TrendRefreshError,
...
export const MAX_CANDIDATE_ATTEMPTS = Math.max(2,Math.min(5,Number(process.env.TOP_KEYWORD_MAX_ATTEMPTS||5)));
export const FINALIZE_RECOVERY_PASSES = 1; // v8.0.28: finalize에서 외부 검색 없이 기존 Fact Ledger 기반 로컬 복구를 1회 수행합니다.
// v8.0.52: 후보 25개를 1개씩 처리하고 자동 3회까지 시도하면 정상 최악 조건도
// 25 + 50 + finalize 단계가 필요합니다. 기존 72회 고정 상한은 정상 작업을 중단시킬 수 있어
// 실행 유형별 기본 예산과 진행 기반 자동 확장, 별도 무진행 반복 차단으로 분리합니다.
export const AUTOMATIC_RUN_STEP_BUDGET = Math.max(96, RESEARCH_POOL_LIMIT * MAX_AUTOMATIC_ATTEMPTS + 20);
export const MANUAL_RUN_STEP_BUDGET = Math.max(160, RESEARCH_POOL_LIMIT * MAX_CANDIDATE_ATTEMPTS + 20);
const CONFIGURED_RUN_STEP_BUDGET = Number(process.env.TOP_REFRESH_MAX_STEPS || 0);
export const MAX_RUN_STEPS = Math.max(AUTOMATIC_RUN_STEP_BUDGET, Math.min(200, Number.isFinite(CONFIGURED_RUN_STEP_BUDGET) ? CONFIGURED_RUN_STEP_BUDGET : 0));
export const HARD_MAX_RUN_STEPS = Math.max(MANUAL_RUN_STEP_BUDGET + 32, Math.min(320, Number(process.env.TOP_REFRESH_HARD_MAX_STEPS || 240)));
export const RUN_STEP_BUDGET_EXTENSION = Math.max(8, Math.min(48, Number(process.env.TOP_REFRESH_STEP_EXTENSION || 24)));
export const MAX_STALLED_RUN_STEPS = Math.max(4, Math.min(20, Number(process.env.TOP_REFRESH_MAX_STALLED_STEPS || 10)));
...
function taskProgress(tasks=[]) {
  ...
  return {
    ...
    skipped:rows.filter(task=>String(task?.status||'')==='skipped').length,
    processing:rows.filter(task=>String(task?.status||'')==='processing').length,
  };
}

function selectionRankOf(trend={},index=0){
  return Math.max(1,Number(trend?.selectionRank||trend?.sourceRank||trend?.rank||index+1));
}

export function buildEarlyCompletionPlan(candidates=[],tasks=[],targetCount=TARGET_TOP_COUNT,attemptLimit=MAX_AUTOMATIC_ATTEMPTS){
  const rows=Array.isArray(candidates)?candidates:[];
  const taskMap=taskMapByCandidate(Array.isArray(tasks)?tasks:[]);
  const prepared=rows.map((trend,index)=>({
    trend,
    index,
    taskId:taskIdentity(trend,index),
    rank:selectionRankOf(trend,index),
    task:taskMap.get(taskIdentity(trend,index))||null,
  })).sort((a,b)=>a.rank-b.rank);
  const ready=prepared.filter(row=>TERMINAL_TASKS.has(String(row.task?.status||'')));
  if(ready.length<Math.max(1,Number(targetCount||TARGET_TOP_COUNT))){
    return {targetReached:false,readyCount:ready.length,cutoffRank:null,retryTaskIds:[],skipTaskIds:[]};
  }
  const cutoff=ready[Math.max(1,Number(targetCount||TARGET_TOP_COUNT))-1];
  const cutoffRank=Number(cutoff?.rank||0);
  const retryTaskIds=prepared
    .filter(row=>row.rank<=cutoffRank&&String(row.task?.status||'')==='retry_wait'&&Number(row.task?.attempts||0)<attemptLimit)
    .map(row=>row.taskId);
  const skipTaskIds=prepared
    .filter(row=>row.rank>cutoffRank&&['queued','retry_wait'].includes(String(row.task?.status||'')))
    .map(row=>row.taskId);
  return {
    targetReached:true,
    ...
    retryTaskIds,
    skipTaskIds,
    remainingRequiredRetries:retryTaskIds.length,
    unnecessaryCandidates:skipTaskIds.length,
  };
}

async function applyEarlyCompletionSkips(runId,candidates=[],tasks=[],plan=null){
  const nextPlan=plan||buildEarlyCompletionPlan(candidates,tasks,TARGET_TOP_COUNT,MAX_AUTOMATIC_ATTEMPTS);
  if(!nextPlan?.targetReached||!nextPlan.skipTaskIds?.length)return Array.isArray(tasks)?tasks:[];
  const candidateMap=new Map((Array.isArray(candidates)?candidates:[]).map((trend,index)=>[taskIdentity(trend,index),trend]));
  const finishedAt=new Date().toISOString();
  for(const taskId of nextPlan.skipTaskIds){
    const trend=candidateMap.get(taskId)||{};
    await updateCronRunTask(runId,taskId,{
      candidateId:taskId,
      slug:trend.slug||taskId,
      publicationStageId:stageIdentity(runId,trend,Math.max(0,Number(trend?.selectionRank||1)-1)),
      status:'skipped',
      error:'TOP20 공개에 필요한 정상 콘텐츠가 이미 확보돼 추가 생성이 필요하지 않습니다.',
      errorCode:'SKIPPED_NOT_NEEDED',
      skipReason:'skipped_not_needed',
      candidatePhase:'skipped_not_needed',
      nextAction:'none',
      finishedAt,
    });
  }
  return getCronRunTasks(runId);
}

function isResearchRetryReason(reason='') {
  return /(?:INSUFFICIENT_KEYWORD_EVIDENCE|키워드의 정체 또는 현재 이슈|확인 자료를 확보하지 못|확인 사실 없음|연결 출처 없음|공식·신뢰 근거 없음|근거가 연결된 사실|출처 조회|source|timeout|timed out|429|5\d\d|network|fetch|dns|tls|connection|lock busy|CONTENT_LOCK_BUSY|stage_not_found|stage_identity_mismatch|STAGE_READ_FAILED|STAGE_WRITE_VERIFY_FAILED|RUN_CONTENT_READ_FAILED|RUN_CONTENT_WRITE_VERIFY_FAILED|CONTENT_KEYWORD_NOT_READY|STRICT_CONTENT_ACCURACY_FAILED|NO_ACCURATE_CONTENT|content_validation|피드 제목|피드 요약|최종 제목|상세 본문|상세 콘텐츠 형식|공개 금지 내부 진단|저작권 유사도|사실 충돌|키워드 설명형 피드 생성 검증 실패)/i.test(String(reason||''));
}

export function shouldRetryCandidate(errorOrReasons) {
...
export function classifyCandidateFailure(errorOrReasons='') {
  const values=Array.isArray(errorOrReasons)
    ? errorOrReasons
    : (typeof errorOrReasons==='string'?[errorOrReasons]:[errorOrReasons?.code,errorOrReasons?.message,...(Array.isArray(errorOrReasons?.details?.reasons)?errorOrReasons.details.reasons:[])]);
  const text=values.filter(Boolean).join(' / ');
  if(/최소\s*1,?000자|상세 본문.*미충족|FEED_DETAIL_MIN|CONTENT_BODY_TOO_SHORT/i.test(text))return 'CONTENT_BODY_TOO_SHORT';
  if(/RUN_CONTENT_WRITE_VERIFY_FAILED|STAGE_WRITE_VERIFY_FAILED|snapshot.*저장|stage.*저장/i.test(text))return 'RUN_SNAPSHOT_WRITE_FAILED';
  if(/stage_not_found|stage·snapshot·slug alias가 모두 없음/i.test(text))return 'RUN_STAGE_NOT_FOUND';
  if(/stage_identity_mismatch/i.test(text))return 'RUN_STAGE_IDENTITY_MISMATCH';
  if(/INSUFFICIENT_KEYWORD_EVIDENCE|키워드의 정체 또는 현재 이슈|확인 자료를 확보하지 못/i.test(text))return 'INSUFFICIENT_KEYWORD_EVIDENCE';
  if(/대표 키워드|확정 키워드가 없습니다|CANONICAL_EVENT_UNRESOLVED/i.test(text))return 'CANONICAL_EVENT_UNRESOLVED';
  if(/STRICT_CONTENT_ACCURACY_FAILED|NO_ACCURATE_CONTENT|정확성 검증/i.test(text))return 'CONTENT_ACCURACY_FAILED';
  if(/CONTENT_LOCK_BUSY|lock busy/i.test(text))return 'CONTENT_LOCK_BUSY';
  if(/timeout|timed out|429|5\d\d|network|fetch|dns|tls|connection/i.test(text))return 'EXTERNAL_RESEARCH_TEMPORARY_FAILURE';
  const explicit=values.map(value=>String(value||'').trim()).find(value=>/^[A-Z][A-Z0-9_]{3,}$/.test(value));
  return explicit||'CONTENT_GENERATION_FAILED';
}
...
function runAgeMs(run={}) {
  // 관리자 재개는 새 실행 시간창으로 계산합니다. 최초 startedAt은 감사 이력으로 유지합니다.
  const started=new Date(run?.resumeWindowStartedAt||run?.resumedAt||run?.startedAt||run?.queuedAt||run?.updatedAt||0).getTime();
  return started>0?Math.max(0,Date.now()-started):0;
}

function defaultRunStepBudget(run={}) {
  const manual=String(run?.manualRetryAllowed||'').toLowerCase()==='true';
  return manual?MANUAL_RUN_STEP_BUDGET:MAX_RUN_STEPS;
}

function progressSignature(run={},tasks=[]) {
  const rows=Array.isArray(tasks)?tasks:[];
  const counts={};
  let attempts=0;
  ...
  return JSON.stringify({
    batchCursor:Number(run?.batchCursor||0),
    lastCompletedCursor:Number(run?.lastCompletedCursor||0),
    retryCursor:Number(run?.retryCursor||0),
    attemptedCandidates:Number(run?.attemptedCandidates||0),
    publishable:Number(run?.publishable||0),
    attempts,
    counts,
  });
}
...
async function assertRunCanContinue(runId,phase,{countStep=false}={}) {
  ...
  if(!compatibility.compatible)throw new TrendRefreshError('top20_new_run_required',`이 실행은 ${compatibility.engineVersion||'이전'} 엔진에서 시작됐고 현재 TOP25 생성·TOP20 공개 정책과 호환되지 않습니다. 기존 작업을 중단하고 새 TOP 작업을 시작하세요.`,{runId,phase,currentEngineVersion:CURRENT_TREND_ENGINE_VERSION,...compatibility});
  ...
  if(countStep){
    ...
    const signature=progressSignature(run,tasks);
    const previousSignature=String(run?.lastProgressSignature||'');
    const progressed=!previousSignature||previousSignature!==signature;
    const stalledStepCount=progressed?0:Number(run?.stalledStepCount||0)+1;
    const next=Number(run.stepCount||0)+1;
    let stepBudget=Math.max(Number(run?.stepBudget||0),defaultRunStepBudget(run));
    let progressExtensions=Number(run?.progressExtensions||0);

    if(stalledStepCount>MAX_STALLED_RUN_STEPS){
      throw new TrendRefreshError('trend_refresh_stalled_loop',`TOP 갱신이 같은 진행 상태에서 ${stalledStepCount}회 반복돼 자동 중단됐습니다. 완료된 후보는 보존되며 관리자에서 중단 지점부터 재개할 수 있습니다.`,{runId,phase,stalledStepCount,maxStalledSteps:MAX_STALLED_RUN_STEPS,stepCount:next,stepBudget});
    }
    if(next>stepBudget){
      if(progressed&&stepBudget<HARD_MAX_RUN_STEPS){
        stepBudget=Math.min(HARD_MAX_RUN_STEPS,Math.max(next,stepBudget+RUN_STEP_BUDGET_EXTENSION));
        progressExtensions+=1;
      }else{
        throw new TrendRefreshError('trend_refresh_step_limit',`TOP 갱신이 단계 예산 ${stepBudget}회를 사용했지만 진행 상태가 갱신되지 않아 자동 중단됐습니다. 완료된 후보는 보존되며 관리자에서 재개할 수 있습니다.`,{runId,phase,maxSteps:stepBudget,hardMaxSteps:HARD_MAX_RUN_STEPS,stalledStepCount,stepCount:next});
      }
    }
    if(next>HARD_MAX_RUN_STEPS){
      throw new TrendRefreshError('trend_refresh_hard_step_limit',`TOP 갱신이 절대 안전 한도 ${HARD_MAX_RUN_STEPS}회를 초과해 자동 중단됐습니다.`,{runId,phase,maxSteps:HARD_MAX_RUN_STEPS,stepCount:next});
    }
    ...
    await patchCronRun(runId,{
      ...
      lastProgressAt:progressed?now:(run?.lastProgressAt||now),
      lastPhase:phase,
      heartbeatAt:now,
    });
    return {...run,stepCount:next,stepBudget,progressExtensions,stalledStepCount,lastProgressSignature:signature};
  }
  return run;
}
...
function isStopCode(code='') {
  return ['trend_refresh_cancelled','trend_refresh_time_limit','trend_refresh_step_limit','trend_refresh_stalled_loop','trend_refresh_hard_step_limit'].includes(String(code));
}
...
async function repairIncompleteStagesLocally(runId,candidates=[],tasks=[]) {
  ...
  const results=await mapLimit(candidates,4,async (trend,index)=>{
    ...
    if(String(task?.status||'')==='skipped')return {candidateId:taskId,slug:trend.slug,repaired:false,skipped:true,reason:'skipped_not_needed'};
    try{
      const verified=await readVerifiedStage(runId,trend,index);
  ...
  return results.filter(row=>!row?.skipped&&(row?.repaired||(!row?.alreadyReady&&row?.reason)));
}
...
function toPublishedTrend(trend = {}, content = {}) {
  ...
  const contentMeta=isUnsplashImageMeta(content.imageMeta)?content.imageMeta:null;
  const trendMeta=isUnsplashImageMeta(trend.imageMeta)?trend.imageMeta:null;
  const imageMeta=contentMeta||trendMeta||null;
  const thumbnail=imageMeta?.thumbUrl||imageMeta?.imageUrl||null;
  const thumbnailSource=imageMeta?'Unsplash':null;
  return {
    ...trend,
...
function normalizedCandidateTerm(value='') {
  if(value&&typeof value==='object')return String(value.keyword||value.value||value.term||'').replace(/\s+/g,' ').trim();
  return String(value||'').replace(/\s+/g,' ').trim();
}

export function buildIndependentResearchTrend(trend = {}, index = 0) {
  const rankedKeyword = String(trend.topKeyword || trend.keyword || trend.rawKeyword || '').replace(/\s+/g, ' ').trim();
  const sourceTitles=(Array.isArray(trend.relatedArticles)?trend.relatedArticles:[])
    .map(row=>String(row?.title||'').replace(/\s+/g,' ').trim()).filter(Boolean).slice(0,8);
  const candidateTerms=[
    ...(Array.isArray(trend.keywordCandidates)?trend.keywordCandidates:[]).map(normalizedCandidateTerm),
    String(trend.researchTopicHint||'').replace(/\s+/g,' ').trim(),
  ].filter(Boolean);
  const identityResolution=resolveTop30Keyword({
    topKeyword:rankedKeyword,
    keyword:rankedKeyword,
    rawKeyword:trend.rawKeyword||rankedKeyword,
    candidateTerms,
    sourceTitles,
  });
  const researchKeyword=String(identityResolution?.keyword||rankedKeyword).replace(/\s+/g,' ').trim();
  return {
    slug: trend.slug,
    ...
    topKeyword: researchKeyword,
    rawKeyword: trend.rawKeyword||rankedKeyword,
    originalRankedKeyword:rankedKeyword,
    researchKeyword,
    displayTitle: researchKeyword,
    searchQuery: researchKeyword,
    identityHints:[...new Set([researchKeyword,rankedKeyword,...candidateTerms].filter(Boolean))].slice(0,8),
    sourceTitleHints:sourceTitles,
    candidateTerms,
    keywordResolution:identityResolution,
    researchTopicHint:String(trend.researchTopicHint||'').trim(),
    topTitle: '',
    topTopic: '',
    ...
    researchIsolation: {
      keywordOnly: true,
      topDiscoveryContextUsed: sourceTitles.length>0?'identity_hints_only':false,
...
async function processTrendCandidate(runId, trend, index, existingTask = null, options = {}) {
  ...
  if(previousAttempts>=candidateAttemptLimit){
    const previousReason=String(existingTask?.error||'').trim();
    const rootCause=String(existingTask?.lastRootCauseCode||classifyCandidateFailure([existingTask?.errorCode,previousReason])).trim();
    const reason=previousReason||`키워드별 최대 시도 횟수 ${candidateAttemptLimit}회를 모두 사용했습니다.`;
    await updateCronRunTask(runId,taskId,{candidateId:taskId,slug:trend.slug,publicationStageId:stageId,status:'failed',error:reason,errorCode:rootCause,lastRootCauseCode:rootCause,attemptLimitReached:true,attemptLimitCode:'KEYWORD_ATTEMPT_LIMIT',finishedAt:new Date().toISOString()});
    return {trend,content:null,ready:false,reasons:[reason],status:'failed',errorCode:rootCause};
  }
  ...
  // v8.0.55: 외부 조사·AI 호출 전에 실행 식별용 최소 checkpoint를 먼저 이중 저장합니다.
  // 이후 요청이 강제 종료돼도 stage_not_found가 아니라 실제 조사/생성 오류와 마지막 원인을 확인할 수 있습니다.
  try{
    await saveTrendRunContentSnapshot(runId,taskId,{
      slug:trend.slug,originalSlug:trend.originalSlug||trend.slug,candidateId:taskId,publicationStageId:stageId,
      keyword:localRepairKeyword(trend),topKeyword:localRepairKeyword(trend),displayTitle:trend.displayTitle||trend.keyword||localRepairKeyword(trend),
      searchQuery:trend.searchQuery||trend.keyword||localRepairKeyword(trend),rawKeyword:trend.rawKeyword||trend.keyword||'',eventKey:trend.eventKey||null,
      category:trend.category||'general',contentVersion:CONTENT_VERSION,status:'researching',visibility:'private',hasContent:false,hasNews:false,
      factLedger:{version:3,sources:[],facts:[],confirmedFacts:[],uncertainties:[],conflicts:[]},evidenceSources:[],sourceItems:[],
      card:{summary:'',why:'',points:[]},blog:'',qa:[],instagramCards:[],checkpointAttempt:attempt,checkpointCreatedAt:startedAt,
    },{stageId,checkpoint:true});
    await updateCronRunTask(runId,taskId,{candidatePhase:'research_checkpoint_saved',checkpointSavedAt:new Date().toISOString()});
  }catch(checkpointError){
    await updateCronRunTask(runId,taskId,{checkpointWriteWarning:String(checkpointError?.message||checkpointError).slice(0,240),checkpointWriteCode:String(checkpointError?.code||'RUN_CHECKPOINT_WRITE_FAILED')});
  }
  ...
  try{
    ...
    if(!stageCheck.ready||!verified||reasons.length||!isFixedKeywordFeedReady(verified||{})){
      ...
      const errorCode=classifyCandidateFailure(unique);
      await updateCronRunTask(runId,taskId,{candidateId:taskId,slug:trend.slug,publicationStageId:stageId,status,error:unique.join(' / '),errorCode,lastRootCauseCode:errorCode,attemptLimitReached:!retryable&&attempt>=candidateAttemptLimit,attemptLimitCode:!retryable&&attempt>=candidateAttemptLimit?'KEYWORD_ATTEMPT_LIMIT':'',finishedAt:new Date().toISOString(),nextAction:retryable?(errorCode==='CONTENT_BODY_TOO_SHORT'?'targeted_fact_rewrite':'additional_search_and_rewrite'):'code_or_manual_review'});
    ...
    // v8.0.49: 생성 함수가 최종 검증에서 예외를 던져도 먼저 저장한 stage 초안을
    // 같은 요청 안에서 Fact Ledger 기반으로 복구합니다. 복구가 성공하면 attempts를 더 쓰지 않습니다.
    if(/CONTENT_KEYWORD_NOT_READY|NO_ACCURATE_CONTENT|STRICT_CONTENT_ACCURACY_FAILED|상세 본문|최소 1,000자|content_validation/i.test(`${code} ${reason}`)) {
      try {
        const repaired=await repairCandidateStageFromVerifiedFacts(runId,trend,index,existingTask,reason);
        if(repaired?.repaired){
          const verifiedRecovery=await readVerifiedStage(runId,trend,index);
          if(verifiedRecovery.ready&&verifiedRecovery.content){
            await updateCronRunTask(runId,taskId,{
              candidateId:taskId,slug:trend.slug,publicationStageId:stageId,status:'generated',feedReady:true,
              error:'',errorCode:'',candidatePhase:'same_attempt_fact_recovery',localRecovery:true,
              snapshotVerifiedAt:new Date().toISOString(),finishedAt:new Date().toISOString(),
            });
            return {trend:toPublishedTrend(trend,verifiedRecovery.content),content:verifiedRecovery.content,ready:true,reasons:[],status:'generated'};
          }
        }
      } catch(recoveryError) {
        console.warn(`[same attempt stage recovery warning] ${trend.slug}:`,recoveryError?.message||recoveryError);
      }
    }

    const retryable=attempt<candidateAttemptLimit&&shouldRetryCandidate(error);
    const status=retryable?'retry_wait':'failed';
    const errorCode=classifyCandidateFailure([code,reason,...(Array.isArray(error?.details?.reasons)?error.details.reasons:[])]);
    await updateCronRunTask(runId,taskId,{candidateId:taskId,slug:trend.slug,publicationStageId:stageId,status,error:reason,errorCode,lastRootCauseCode:errorCode,attemptLimitReached:!retryable&&attempt>=candidateAttemptLimit,attemptLimitCode:!retryable&&attempt>=candidateAttemptLimit?'KEYWORD_ATTEMPT_LIMIT':'',candidatePhase:'generation_failed',finishedAt:new Date().toISOString(),nextAction:retryable?(errorCode==='CONTENT_BODY_TOO_SHORT'?'targeted_fact_rewrite':'saved_stage_repair_or_additional_search'):'code_or_manual_review'});
...
export async function startTrendRefreshRun(runId, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  try {
    ...
    let thumbnailPoolStatus=null;
    let thumbnailBackfill=null;
    try{
      thumbnailPoolStatus=await ensureThumbnailPoolReady();
      if(thumbnailPoolStatus?.ready)thumbnailBackfill=await backfillMissingTopThumbnails({limit:TARGET_TOP_COUNT});
      await patchCronRun(runId,{
        thumbnailPoolReady:thumbnailPoolStatus?.ready?'true':'false',
        thumbnailPoolComplete:thumbnailPoolStatus?.complete?'true':'false',
        thumbnailPoolCount:Number(thumbnailPoolStatus?.items?.length||0),
        thumbnailPoolReason:String(thumbnailPoolStatus?.reason||''),
        thumbnailBackfilled:Number(thumbnailBackfill?.updated||0),
        heartbeatAt:new Date().toISOString(),
      });
      await addAudit('thumbnail_pool_preflight','',null,{runId,ready:Boolean(thumbnailPoolStatus?.ready),complete:Boolean(thumbnailPoolStatus?.complete),count:Number(thumbnailPoolStatus?.items?.length||0),backfill:thumbnailBackfill},'TOP 생성 전 사전 이미지 풀 확인·기존 TOP 누락 이미지 보완','system');
    }catch(thumbnailError){
      thumbnailPoolStatus={ready:false,complete:false,items:[],reason:String(thumbnailError?.message||thumbnailError)};
      await patchCronRun(runId,{thumbnailPoolReady:'false',thumbnailPoolReason:thumbnailPoolStatus.reason,heartbeatAt:new Date().toISOString()});
      await addAudit('thumbnail_pool_preflight','',null,{runId,ready:false,error:thumbnailPoolStatus.reason},'이미지 풀 오류는 TOP 선정·콘텐츠 공개를 중단하지 않음','system','failed',thumbnailPoolStatus.reason);
    }
    ...
    await patchCronRun(runId, {
      ...
      // Hybrid publication is calculated only during finalize. A new run has no publication result yet.
      freshReadyCount: 0,
      carryoverReadyCount: 0,
      publicationMode: 'pending',
    });
    ...
    // v8.0.54: 관심도 상대순위 상위 25개는 최대 예비 후보 풀로 고정합니다.
    // 1위부터 순서대로 처리하고 정상 콘텐츠 20개가 확보되면 낮은 순위의 남은 후보는 생성하지 않습니다.
    // 이미 처리된 더 높은 순위 retry_wait 후보만 제한 재시도한 뒤 원래 관심도 순서대로 TOP20을 공개합니다.
...
async function queueRetryOrFinalize(runId, trigger, candidates, tasks, cursor=0) {
  ...
  let earlyPlan=buildEarlyCompletionPlan(candidates,working,TARGET_TOP_COUNT,attemptLimit);
  if(earlyPlan.targetReached&&earlyPlan.skipTaskIds.length){
    working=await applyEarlyCompletionSkips(runId,candidates,working,earlyPlan);
    earlyPlan=buildEarlyCompletionPlan(candidates,working,TARGET_TOP_COUNT,attemptLimit);
  }

  const retryIds=earlyPlan.targetReached?new Set(earlyPlan.retryTaskIds):null;
  const eligible=working.filter(task=>{
    if(String(task?.status||'')!=='retry_wait'||Number(task?.attempts||0)>=attemptLimit)return false;
    const taskId=String(task?.candidateId||task?.slug||'');
    return !retryIds||retryIds.has(taskId);
  });
  if(eligible.length){
    ...
    await patchCronRun(runId,{
      status:'retry_queued',batchCursor:cursor,retryCursor,
      attemptedCandidates:progress.attempted,publishable:progress.ready,retryQueued:eligible.length,
      earlyCompletionReady:earlyPlan.targetReached?'true':'false',
      earlyCompletionCutoffRank:earlyPlan.cutoffRank||0,
      skippedNotNeeded:progress.skipped,
      retryMessageId:queued.messageId||'',heartbeatAt:new Date().toISOString(),
    });
    return {success:true,runId,phase:'retry',cursor:retryCursor,readyCount:progress.ready,retryCount:eligible.length,earlyCompletion:earlyPlan.targetReached,cutoffRank:earlyPlan.cutoffRank||0,messageId:queued.messageId||''};
  }
  ...
  await patchCronRun(runId,{
    status:'finalize_queued',batchCursor:cursor,attemptedCandidates:progress.attempted,publishable:progress.ready,
    manualRetryAllowed:'false',earlyCompletionReady:earlyPlan.targetReached?'true':'false',
    earlyCompletionCutoffRank:earlyPlan.cutoffRank||0,skippedNotNeeded:progress.skipped,
    finalizeMessageId:queued.messageId||'',heartbeatAt:new Date().toISOString(),
  });
  return {success:true,runId,phase:'finalize',cursor,readyCount:progress.ready,failedCount:progress.failed,skippedCount:progress.skipped,earlyCompletion:earlyPlan.targetReached,cutoffRank:earlyPlan.cutoffRank||0,messageId:queued.messageId||''};
}
...
export async function executeTrendRefreshBatch(runId, cursor = 0, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  ...
  try {
    ...
    // TOP20 정상 콘텐츠가 확보되면 25개 전체를 끝까지 생성하지 않습니다.
    // 다만 이미 처리된 더 높은 순위의 retry_wait 후보는 최종 순위에 영향을 줄 수 있어 제한 재시도를 먼저 마칩니다.
    if(progress.ready>=TARGET_TOP_COUNT)return queueRetryOrFinalize(runId,trigger,candidates,refreshedTasks,nextCursor);
...
export async function executeTrendRefreshRetryBatch(runId, cursor = 0, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  ...
  try{
    ...
    // 재시도 한 건으로 TOP20이 확보되면 낮은 순위의 남은 재시도는 즉시 생략합니다.
    // TOP20 안쪽의 더 높은 순위 retry_wait만 queueRetryOrFinalize가 선별해 이어갑니다.
  ...
  try {
    ...
    async function collectReadyRows(){
      const evaluations=await mapLimit(candidates,6,async (trend,index)=>{
        const taskId=taskIdentity(trend,index);
        const task=taskMap.get(taskId);
        if(String(task?.status||'')==='skipped'){
          return {trend,task,ready:false,skipped:true,code:task?.errorCode||'SKIPPED_NOT_NEEDED',reasons:[task?.skipReason||'skipped_not_needed']};
        }
        try{
      ...
      return {
        rows,
        allReady:rows,
        missing:evaluations.filter(row=>!row.ready&&!row.skipped),
        skipped:evaluations.filter(row=>row.skipped),
        duplicateWarnings,
      };
    }
    ...
    const skipped = refreshedTasks
      .filter(task=>String(task?.status||'')==='skipped')
      .map(task=>({
        candidateId:String(task?.candidateId||task?.slug||''),
        slug:String(task?.slug||''),
        title:String(task?.title||''),
        reason:String(task?.skipReason||'skipped_not_needed'),
        code:String(task?.errorCode||'SKIPPED_NOT_NEEDED'),
      }));
    const processedCount = refreshedTasks.filter(task => Number(task?.attempts||0)>0).length;

    // v8.0.51+: 새 실행에서 성공한 후보를 우선 공개하되, 최대 5개가 부족할 때는
    // 직전 정상 TOP의 마지막 공개본을 중복 없이 이어받아 TOP20을 완성합니다.
    // 신규 성공 후보가 15개 미만이면 과거 항목 비중이 지나치게 커지므로 기존 TOP을 유지합니다.
    const freshPublicationRows = readyRows
    ...
    const carryoverRows=[];
    if(freshPublicationRows.length>=MIN_FRESH_PUBLICATION_COUNT&&freshPublicationRows.length<TARGET_TOP_COUNT){
      const needed=Math.min(MAX_PREVIOUS_TOP_CARRYOVER,TARGET_TOP_COUNT-freshPublicationRows.length);
      const candidatesForCarryover=carryoverTrendCandidates(freshPublicationRows,previous,previous.length);
      for(const previousTrend of candidatesForCarryover){
        if(carryoverRows.length>=needed)break;
        const slug=String(previousTrend?.slug||'').trim();
        if(!slug)continue;
        let content=null;
        // 마지막 정상 공개 archive를 우선 사용합니다. includePrivate 조회는 실패한 live 콘텐츠가 존재하면 archive fallback을 건너뜁니다.
        try{content=await getContent(slug);}catch{}
        if(!content||!feedDraftIsReady(content))continue;
        const status=String(content?.status||'published');
        const visibility=String(content?.visibility||'published');
        if(status!=='published'||['private','trashed','hidden_top'].includes(visibility))continue;
        carryoverRows.push({
          trend:{
            ...previousTrend,
            carryoverFromPrevious:true,
            retainedFromPreviousTop:true,
            publicationOrigin:'previous_top_carryover',
            previousPublicRank:Number(previousTrend?.rank||0)||null,
          },
          sourceTrend:previousTrend,
          content:{...content,carryoverFromPrevious:true,publicationOrigin:'previous_top_carryover'},
          feedReady:true,
          ready:true,
          code:'previous_top_carryover',
          reasons:[],
          carryover:true,
        });
      }
    }
    const hybridPublication=buildHybridPublicationRows(freshPublicationRows,carryoverRows,TARGET_TOP_COUNT);
    const publicationRows=hybridPublication.rows;
    const topCountComplete=hybridPublication.complete;
    ...
    const progressiveRecovery = hybridPublication.carryoverCount>0;
    ...
    const executionReport = {
      ...
      contentStaging: {
        ...
        freshReadyCount: hybridPublication.freshCount,
        feedReadyCount: readyRows.filter(row=>row.feedReady===true).length,
        carryoverReadyCount: hybridPublication.carryoverCount,
        publicationMode: hybridPublication.mode,
    ...
    if (!health.healthy) {
      const failureCode=publicationRows.length<TARGET_TOP_COUNT?'top20_hybrid_publication_incomplete':'top20_feed_quality_invalid';
      ...
      const failureMessage=publicationRows.length<TARGET_TOP_COUNT
        ? `상위 25개 생성 후보 중 신규 정상 콘텐츠 ${hybridPublication.freshCount}개와 직전 TOP 이어받기 ${hybridPublication.carryoverCount}개를 합쳐 ${publicationRows.length}/${TARGET_TOP_COUNT}개만 준비돼 기존 TOP을 유지했습니다. 신규 콘텐츠가 최소 ${MIN_FRESH_PUBLICATION_COUNT}개 이상이고 부족분이 최대 ${MAX_PREVIOUS_TOP_CARRYOVER}개 이내일 때만 혼합 공개합니다.${missingLabels.length?` 미완료 항목 — ${missingLabels.join(' | ')}`:''}`
    ...
    const finalTrends = publicationRows.slice(0, TARGET_TOP_COUNT).map((row, index) => {
      const carryover=Boolean(row?.carryover||row?.trend?.carryoverFromPrevious);
      const sourceRank=carryover?null:Number(row.trend?.selectionRank||row.trend?.rank||index+1);
      return {
        ...row.trend,
        selectionRank:carryover?null:sourceRank,
        sourceRank,
        promotedFromReserve:carryover?false:sourceRank>TARGET_TOP_COUNT,
        carryoverFromPrevious:carryover,
        retainedFromPreviousTop:carryover,
        publicationOrigin:carryover?'previous_top_carryover':'fresh_candidate',
        rank:index+1,
      };
    });
    ...
    await saveTrendCandidateReport({
      ...executionReport,
      refreshFailed: false,
      degraded: hybridPublication.carryoverCount>0,
      refreshSummary,
    ...
    await patchCronRun(runId, {
      ...
      freshPublished: hybridPublication.freshCount,
      carryoverPublished: hybridPublication.carryoverCount,
      publicationMode: hybridPublication.mode,
    ...
    if(isStopCode(error?.code))return stopRun(runId,error,{actor,trigger});
    const terminalCodes = new Set(['abnormal_top_pool_shrink','top20_from25_content_incomplete','top20_hybrid_publication_incomplete','top20_feed_quality_invalid','top20_incomplete','trend_run_not_found','trend_candidates_missing','trend_refresh_lock_lost','top20_new_run_required']);
    if (terminalCodes.has(String(error?.code || ''))) return failRun(runId, error, { actor, trigger });
