...
import { PUBLIC_TOP_COUNT, TOP_POLICY_VERSION } from './topConfig';
...
export const RESEARCH_POOL_LIMIT = PUBLIC_TOP_COUNT;
export const TREND_REFRESH_STEP_BATCH_SIZE = 3;
export const TREND_REFRESH_RETRY_BATCH_SIZE = 6;
const STEP_BATCH_SIZE = TREND_REFRESH_STEP_BATCH_SIZE;
const RETRY_BATCH_SIZE = TREND_REFRESH_RETRY_BATCH_SIZE;
const MAX_AUTOMATIC_ATTEMPTS = Math.min(2, Math.max(1, Number(process.env.TOP_KEYWORD_AUTO_ATTEMPTS || 2)));
...
export const MAX_RUN_STEPS = Math.max(12,Math.min(40,Number(process.env.TOP_REFRESH_MAX_STEPS||18)));
export const MAX_RUN_DURATION_MS = Math.max(15*60*1000,Math.min(3*60*60*1000,Number(process.env.TOP_REFRESH_MAX_MINUTES||60)*60*1000));
...
function isResearchRetryReason(reason='') {
  return /(?:확인 사실 없음|연결 출처 없음|공식·신뢰 근거 없음|근거가 연결된 사실|출처 조회|source|timeout|timed out|429|5\d\d|network|fetch|dns|tls|connection|lock busy|CONTENT_LOCK_BUSY|stage_not_found|STAGE_READ_FAILED|STAGE_WRITE_VERIFY_FAILED|CONTENT_KEYWORD_NOT_READY|content_validation|피드 제목|피드 요약|최종 제목|상세 본문|상세 콘텐츠 형식|공개 금지 내부 진단|저작권 유사도|사실 충돌|키워드 설명형 피드 생성 검증 실패)/i.test(String(reason||''));
}
...
function normalizedStageKeyword(value='') {
  return String(value||'').toLowerCase().replace(/[^0-9a-z가-힣]/g,'');
}

function stageMatchesTrend(content={},trend={}) {
  const expected=normalizedStageKeyword(localRepairKeyword(trend));
  const actual=normalizedStageKeyword(content?.topKeyword||content?.keyword||content?.displayTitle||'');
  return Boolean(expected&&actual&&expected===actual&&Number(content?.contentVersion||0)===CONTENT_VERSION);
}

async function readVerifiedStage(runId, trend, index=0) {
  const stageId=stageIdentity(runId,trend,index);
  let content=await getPublicationStage(stageId,{retries:2,throwOnError:true});

  // getCachedContent가 결과를 만든 뒤 호출 제한시간이 끝나면 slug stage만 남고
  // 실행별 stage 저장 직전에 종료될 수 있습니다. 같은 키워드·콘텐츠 버전의 slug stage를
  // 실행별 stage로 승격해 stage_not_found를 복구합니다.
  if(!content&&trend?.slug&&String(trend.slug)!==stageId){
    const legacy=await getPublicationStage(trend.slug,{retries:2,throwOnError:false});
    if(legacy&&stageMatchesTrend(legacy,trend)){
      content=await savePublicationStage({...legacy,slug:trend.slug,candidateId:taskIdentity(trend,index),publicationStageId:stageId,stageRecoveredFrom:'legacy_slug_stage'},{stageId});
    }
  }

  if(!content)return {ready:false,code:'stage_not_found',content:null,reasons:['stage_not_found']};
  const reasons=stageValidationReasons(content);
  if(reasons.length||!isFixedKeywordFeedReady(content||{}))return {ready:false,code:'stage_validation_failed',content,reasons:[...new Set(reasons.length?reasons:['content_validation_failed'])]};
  return {ready:true,code:'ready',content,reasons:[]};
}
...
async function repairCandidateStageFromVerifiedFacts(runId,trend,index,task=null,reason='') {
  ...
  let source='stage';
  try{sourceContent=await getPublicationStage(stageId,{retries:2,throwOnError:false});}catch{}
  if(!sourceContent&&trend?.slug&&String(trend.slug)!==stageId){
    source='legacy_slug_stage';
    try{
      const legacy=await getPublicationStage(trend.slug,{retries:2,throwOnError:false});
      if(legacy&&stageMatchesTrend(legacy,trend))sourceContent=legacy;
    }catch{}
  }
  if(!sourceContent){
    source='published_content';
    try{sourceContent=await getContent(trend.slug,{includePrivate:true});}catch{}
  }
  if(!sourceContent)return {repaired:false,taskId,stageId,reason:'repair_source_missing'};
  const stats=localRepairEvidenceStats(sourceContent);
  if(stats.facts<1||stats.evidence<1)return {repaired:false,taskId,stageId,reason:'repair_evidence_missing',stats};
  ...
  try{
    await savePublicationStage({...repaired,slug:trend.slug,candidateId:taskId,publicationStageId:stageId,originalSlug:trend.originalSlug||trend.slug,localRecovery:true,localRecoverySource:source},{stageId});
    ...
    await updateCronRunTask(runId,taskId,{
      ...
      error:'',errorCode:'',recoveredFromFactLedger:true,recoverySource:source,recoveryReason:reason||'',finishedAt:new Date().toISOString(),
    });
...
async function processTrendCandidate(runId, trend, index, existingTask = null, options = {}) {
  ...
  if(existingTask&&!forceRetry&&['generated','reused'].includes(existingTask.status)){
    try{
      const verified=await readVerifiedStage(runId,trend,index);
      if(verified.ready)return {trend:toPublishedTrend(trend,verified.content),content:verified.content,ready:true,status:existingTask.status,reasons:[]};
    }catch(error){
      // generated로 기록됐어도 stage 조회 오류/누락이면 아래 생성 경로에서 복구합니다.
    }
  }
  ...
  await updateCronRunTask(runId,taskId,{
    ...
    status:'processing',attempts:attempt,startedAt,title:trend.displayTitle||trend.keyword,error:'',errorCode:'',
  });
  ...
  try{
    const independentTrend=buildIndependentResearchTrend({...trend,candidateId:taskId,publicationStageId:stageId,researchAttempt:attempt,researchMode:attempt===1?'keyword_identity_and_current_issue':'expanded_keyword_research'},index);
    ...
    const saved=await savePublicationStage({...normalizedGenerated,slug:trend.slug,candidateId:taskId,publicationStageId:stageId,originalSlug:trend.originalSlug||trend.slug},{stageId});
    const verified=await getPublicationStage(stageId,{retries:2,throwOnError:true});
    const reasons=stageValidationReasons(verified||saved||{});
    if(!verified||reasons.length||!isFixedKeywordFeedReady(verified||{})){
    ...
    await updateCronRunTask(runId,taskId,{candidateId:taskId,slug:trend.slug,publicationStageId:stageId,status:finalStatus,feedReady:true,error:'',errorCode:'',finishedAt:new Date().toISOString()});
    ...
    await updateCronRunTask(runId,taskId,{candidateId:taskId,slug:trend.slug,publicationStageId:stageId,status,error:reason,errorCode,finishedAt:new Date().toISOString(),nextAction:retryable?'additional_search_and_rewrite':'code_or_manual_review'});
...
export async function startTrendRefreshRun(runId, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  try {
    ...
    if(persistedCandidates.length&&String(existingRun?.workflowType||'')==='top_refresh_v2'){
      const requiresTop20Migration=persistedCandidates.length!==TARGET_TOP_COUNT||persistedCandidates.some(candidate=>candidate?.fixedTop20!==true);
      if(requiresTop20Migration){
        throw new TrendRefreshError(
          'top20_new_run_required',
          `이 실행은 이전 TOP${persistedCandidates.length} 기준으로 저장돼 TOP${TARGET_TOP_COUNT} 원자적 공개에 사용할 수 없습니다. 기존 작업을 중단하고 새 TOP 갱신을 시작하세요.`,
    ...
    // v8.0.25: TOP 키워드는 v8.0.20 방식으로 관심도 상대순위에서 먼저 20개를 확정합니다.
    // 상세·피드 생성 가능 여부는 TOP 선정 조건이 아니며 21위 이하 후보로 교체하지 않습니다.
    // 확정된 같은 20개 키워드를 독립 검색하여 피드·요약정보·제목을 완성합니다.
    const selectedTop20 = prepareSelectedTopCandidates((prepared.trends || []).slice(0, TARGET_TOP_COUNT),runId,TARGET_TOP_COUNT);
    if (selectedTop20.length !== TARGET_TOP_COUNT) {
      throw new TrendRefreshError('abnormal_top_shrink', `관심도 기준 TOP 키워드가 ${selectedTop20.length}/${TARGET_TOP_COUNT}개만 확정되어 기존 공개 TOP을 유지했습니다.`, {
        targetTopCount: TARGET_TOP_COUNT,
        selectedCount: selectedTop20.length,
        preparedCount: Array.isArray(prepared.trends) ? prepared.trends.length : 0,
      });
    }
    const candidates = selectedTop20.map((item, index) => ({
      ...item,
      rank: index + 1,
      visibility: 'private',
      publicTopPolicy: TOP_POLICY_VERSION,
    ...
    await saveTrendCandidateReport({
      ...
      runId,
      refreshFailed: false,
      workflow: 'bounded_qstash_batches_with_local_fact_recovery',
      contentStaging: {
        targetTopCount: TARGET_TOP_COUNT,
    ...
    const terminalCodes = new Set(['no_research_candidates','abnormal_top_shrink','trend_refresh_in_progress','trend_lock_unavailable']);
...
export async function executeTrendRefreshBatch(runId, cursor = 0, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  ...
  try {
    ...
    const locked=await acquireLock(lockName,360);
    if(!locked)return {success:true,runId,phase:'batch',cursor:safeCursor,idempotent:true,stepAlreadyRunning:true};
...
export async function executeTrendRefreshRetryBatch(runId, cursor = 0, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  ...
  try{
    ...
    const locked=await acquireLock(lockName,360);
    if(!locked)return {success:true,runId,phase:'retry',cursor:safeCursor,idempotent:true,stepAlreadyRunning:true};
...
export async function finalizeTrendRefreshRun(runId, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  try {
    ...
    // 고정 TOP20 흐름에는 후순위 후보가 없으므로 skipped 상태를 만들지 않습니다.
    // 복구 후에도 완료되지 않은 키워드는 실제 실패·검토 상태를 유지해 원인을 숨기지 않습니다.
    const refreshedTasks = await getCronRunTasks(runId);
    ...
    // v8.0.14: TOP20 키워드는 순위 단계에서 이미 고정됐습니다.
    // 여기서는 같은 20개 키워드의 상세·피드·제목 완성 여부만 확인합니다.
    const publicationRows = readyRows.slice(0, TARGET_TOP_COUNT);
    ...
    if (!health.healthy) {
      const failureCode=readyRows.length<TARGET_TOP_COUNT?'top20_fixed_content_incomplete':'top20_feed_quality_invalid';
      ...
      const failureMessage=readyRows.length<TARGET_TOP_COUNT
        ? `확정된 TOP20 키워드 중 상세·피드·제목이 ${readyRows.length}/${TARGET_TOP_COUNT}만 완료되어 기존 공개 TOP을 유지했습니다.${missingLabels.length?` 미완료 항목 — ${missingLabels.join(' | ')}`:''}`
    ...
    const finalTrends = publicationRows.slice(0, TARGET_TOP_COUNT).map((row, index) => ({ ...row.trend, rank: index + 1 }));
    ...
    if(isStopCode(error?.code))return stopRun(runId,error,{actor,trigger});
    const terminalCodes = new Set(['abnormal_top_shrink','top20_fixed_content_incomplete','top20_feed_quality_invalid','top20_incomplete','trend_run_not_found','trend_candidates_missing','trend_refresh_lock_lost','top20_new_run_required']);
    if (terminalCodes.has(String(error?.code || ''))) return failRun(runId, error, { actor, trigger });
