...
export const TARGET_TOP_COUNT = 30;
export const RESEARCH_POOL_LIMIT = 30;
...
async function readVerifiedStage(runId, trend, index=0) {
  ...
  const content=await getPublicationStage(stageId,{retries:2,throwOnError:true});
...
async function processTrendCandidate(runId, trend, index, existingTask = null, options = {}) {
  ...
  try{
    ...
    const generated=await getCachedContent(
      ...
      independentTrend,
      {force:forceRetry||attempt>1,stageOnly:true,fixedTop30Flow:true},
    );
    ...
    // API 내부 legacy slug stage와 별개로 실행별 고유 stage에 다시 저장하고 읽기 검증합니다.
    const saved=await savePublicationStage({...generated,slug:trend.slug,candidateId:taskId,publicationStageId:stageId,originalSlug:trend.originalSlug||trend.slug},{stageId});
...
export async function startTrendRefreshRun(runId, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  try {
    ...
    // v8.0.25: TOP 키워드는 v8.0.20 방식으로 관심도 상대순위에서 먼저 30개를 확정합니다.
    // 상세·피드 생성 가능 여부는 TOP 선정 조건이 아니며 31위 이하 후보로 교체하지 않습니다.
    // 확정된 같은 30개 키워드를 독립 검색하여 피드·요약정보·제목을 완성합니다.
    const selectedTop30 = prepareSelectedTopCandidates((prepared.trends || []).slice(0, TARGET_TOP_COUNT),runId,TARGET_TOP_COUNT);
    if (selectedTop30.length !== TARGET_TOP_COUNT) {
      throw new TrendRefreshError('abnormal_top_shrink', `관심도 기준 TOP 키워드가 ${selectedTop30.length}/30개만 확정되어 기존 공개 TOP을 유지했습니다.`, {
        targetTopCount: TARGET_TOP_COUNT,
        selectedCount: selectedTop30.length,
        preparedCount: Array.isArray(prepared.trends) ? prepared.trends.length : 0,
      });
    }
    const candidates = selectedTop30.map((item, index) => ({
      ...item,
      rank: index + 1,
      visibility: 'private',
      publicTopPolicy: 'fixed_keyword_content_v15',
...
async function normalizeReadyStagesForBoundedRetry(runId,candidates=[],tasks=[],attemptLimit=MAX_AUTOMATIC_ATTEMPTS){
  ...
  // 실제 중복 여부는 30개 전체 집합에서 키워드·Fact Ledger·출처 중복까지 함께 평가합니다.
...
export async function executeTrendRefreshBatch(runId, cursor = 0, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  ...
  try {
    ...
    if(isStopCode(error?.code))return stopRun(runId,error,{actor,trigger});
    const terminalCodes = new Set(['trend_run_not_found','trend_candidates_missing','trend_refresh_lock_lost']);
    if (terminalCodes.has(String(error?.code || ''))) return failRun(runId, error, { actor, trigger });
...
export async function finalizeTrendRefreshRun(runId, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  try {
    ...
    // 고정 TOP30 흐름에는 후순위 후보가 없으므로 skipped 상태를 만들지 않습니다.
    ...
    // v8.0.14: TOP30 키워드는 순위 단계에서 이미 고정됐습니다.
    // 여기서는 같은 30개 키워드의 상세·피드·제목 완성 여부만 확인합니다.
    ...
    const executionReport = {
      ...
      contentStaging: {
        ...
        rejected:rejected.slice(0,30).map(row=>({candidateId:row.trend?.candidateId||'',slug:row.trend?.slug,title:row.trend?.displayTitle,reasons:row.reasons})),
    ...
    if (!health.healthy) {
      const failureCode=readyRows.length<TARGET_TOP_COUNT?'top30_fixed_content_incomplete':'top30_feed_quality_invalid';
      ...
      const failureMessage=readyRows.length<TARGET_TOP_COUNT
        ? `확정된 TOP30 키워드 중 상세·피드·제목이 ${readyRows.length}/30만 완료되어 기존 공개 TOP을 유지했습니다.${missingLabels.length?` 미완료 항목 — ${missingLabels.join(' | ')}`:''}`
        : 'TOP30 피드 품질 검증을 통과하지 못해 기존 공개 TOP을 유지했습니다.';
    ...
    if (publicationRows.length !== TARGET_TOP_COUNT) {
      throw new TrendRefreshError('top30_incomplete', `검증된 TOP이 ${publicationRows.length}개만 준비되어 30개 공개를 완료하지 못했습니다.`, {
        targetTopCount: TARGET_TOP_COUNT,
    ...
    if(isStopCode(error?.code))return stopRun(runId,error,{actor,trigger});
    const terminalCodes = new Set(['abnormal_top_shrink','top30_fixed_content_incomplete','top30_feed_quality_invalid','top30_incomplete','trend_run_not_found','trend_candidates_missing','trend_refresh_lock_lost']);
    if (terminalCodes.has(String(error?.code || ''))) return failRun(runId, error, { actor, trigger });
