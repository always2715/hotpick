...
import {
  ...
  isPreviousTopCarryoverContentReady,
...
export function buildIndependentResearchTrend(trend = {}, index = 0) {
  ...
  return {
    ...
    priorResearchCheckpoint:trend.priorResearchCheckpoint||null,
    researchIsolation: {
      keywordOnly: true,
...
async function processTrendCandidate(runId, trend, index, existingTask = null, options = {}) {
  ...
  try{
    let priorResearchCheckpoint=null;
    try{
      priorResearchCheckpoint=await getTrendRunContentSnapshot(runId,taskId,{slug:trend.slug,stageId,retries:1,throwOnError:false});
      if(!priorResearchCheckpoint)priorResearchCheckpoint=await getPublicationStage(stageId,{retries:1,throwOnError:false});
    }catch{}
    const compactPriorCheckpoint=priorResearchCheckpoint&&priorResearchCheckpoint.factLedger?{
      factLedger:priorResearchCheckpoint.factLedger,
      evidenceSources:Array.isArray(priorResearchCheckpoint.evidenceSources)?priorResearchCheckpoint.evidenceSources:Array.isArray(priorResearchCheckpoint.sourceItems)?priorResearchCheckpoint.sourceItems:[],
      sourceItems:Array.isArray(priorResearchCheckpoint.sourceItems)?priorResearchCheckpoint.sourceItems:Array.isArray(priorResearchCheckpoint.evidenceSources)?priorResearchCheckpoint.evidenceSources:[],
      relatedNews:Array.isArray(priorResearchCheckpoint.relatedNews)?priorResearchCheckpoint.relatedNews:[],
      relatedContent:Array.isArray(priorResearchCheckpoint.relatedContent)?priorResearchCheckpoint.relatedContent:[],
    }:null;
    const independentTrend=buildIndependentResearchTrend({...trend,runId,candidateId:taskId,publicationStageId:stageId,researchAttempt:attempt,researchMode:attempt===1?'keyword_identity_and_current_issue':'expanded_keyword_research',priorResearchCheckpoint:compactPriorCheckpoint},index);
    ...
    await updateCronRunTask(runId,taskId,{candidateId:taskId,slug:trend.slug,publicationStageId:stageId,status,error:reason,errorCode,candidatePhase:'generation_failed',finishedAt:new Date().toISOString(),nextAction:retryable?'saved_stage_repair_or_additional_search':'code_or_manual_review'});
...
export async function startTrendRefreshRun(runId, { actor = 'qstash', trigger = 'external_cron' } = {}) {
  try {
    ...
    // v8.0.55: 관심도 상대순위 상위 25개는 최대 예비 후보 풀로 고정합니다.
  ...
  try {
    ...
    if(freshPublicationRows.length>=MIN_FRESH_PUBLICATION_COUNT&&freshPublicationRows.length<TARGET_TOP_COUNT){
      ...
      for(const previousTrend of candidatesForCarryover){
        ...
        if(!slug)continue;
        let content=null;
        try{content=await getContent(slug,{includePrivate:true});}catch{}
        if(!content||!(feedDraftIsReady(content)||isPreviousTopCarryoverContentReady(content)))continue;
        carryoverRows.push({
          trend:{
            ...previousTrend,
