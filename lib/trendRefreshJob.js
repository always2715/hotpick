...
import { stageMatchesTrend } from './stageIdentityPolicy';
...
async function readVerifiedStage(runId, trend, index=0) {
  ...
  // 1순위는 실행별 durable snapshot입니다. publication_stage가 사라져도 이 원본으로 복구합니다.
  try{addSource('run_snapshot',await getTrendRunContentSnapshot(runId,taskId,{retries:2,throwOnError:false}));}catch{}
  ...
  for(const row of sources){
    if(!stageMatchesTrend(row.content,trend,runId,index,CONTENT_VERSION))continue;
    ...
    // 일반화 Fact, 제목·요약 형식 문제 등은 같은 검증 Fact Ledger로 즉시 재구성합니다.
    // 외부 검색을 다시 호출하지 않으며, 구체적인 사실과 출처가 없으면 통과시키지 않습니다.
    if(reasons.length||!isFixedKeywordFeedReady(candidate)){
      const repaired=buildFactBasedStageCandidate(
        ...
        reasons.join(' / ')||'실행별 콘텐츠 최종 검증 복구',
      );
      if(repaired){
        candidate={...repaired,slug:trend.slug,candidateId:taskId,publicationStageId:stageId,originalSlug:trend.originalSlug||trend.slug,stageRecoveredFrom:row.name};
    ...
    if(!reasons.length&&isFixedKeywordFeedReady(candidate)){
      ...
      return {ready:true,code:'ready',content:persisted,reasons:[],source:row.name};
    }
    if(!firstInvalid)firstInvalid={content:candidate,reasons:[...new Set(reasons.length?reasons:['content_validation_failed'])],source:row.name};
  }

  if(firstInvalid)return {ready:false,code:'stage_validation_failed',content:firstInvalid.content,reasons:firstInvalid.reasons,source:firstInvalid.source};
  if(sources.length){
    ...
    return {ready:false,code:'stage_identity_mismatch',content:null,reasons:[`저장된 콘텐츠는 있으나 후보 식별자가 일치하지 않음 (${found.join(' | ')})`]};
  }
  return {ready:false,code:'stage_not_found',content:null,reasons:['실행별 stage와 durable snapshot 모두 없음']};
}
...
async function repairCandidateStageFromVerifiedFacts(runId,trend,index,task=null,reason='') {
  ...
  let source='run_snapshot';
  try{sourceContent=await getTrendRunContentSnapshot(runId,taskId,{retries:2,throwOnError:false});}catch{}
  if(!sourceContent){
    source='stage';
  ...
  if(!sourceContent&&trend?.slug&&String(trend.slug)!==stageId){
    ...
    try{
      ...
      if(legacy&&stageMatchesTrend(legacy,trend,runId,index,CONTENT_VERSION))sourceContent=legacy;
    }catch{}
  }
  if(!sourceContent){
