...
async function assertRunCanContinue(runId,phase,{countStep=false}={}) {
  ...
  if(String(run?.engineVersion||'')&&String(run.engineVersion)!=='8.0.35')throw new TrendRefreshError('top20_new_run_required',`이 실행은 ${run.engineVersion} 엔진에서 시작돼 v8.0.35 정확성 복구 구조로 재개할 수 없습니다. 기존 작업을 중단하고 새 TOP20 작업을 시작하세요.`,{runId,phase,engineVersion:run.engineVersion});
