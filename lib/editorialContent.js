...
export function buildVerifiedFallback(topicTitle, ledger={}, sourceWindowHours=36, contentTier='standard') {
  const profileFacts=distinctFacts(accurateFacts(ledger,{scope:'profile',limit:14,allowSingleTrusted:true}),14);
  const issueFacts=distinctFacts(accurateFacts(ledger,{scope:'issue',limit:20,allowSingleTrusted:true}),20);
  ...
  const exactFactSentence=fact=>sanitizePublicText(politeFactSentence(fact,topicTitle,'')).slice(0,320);
  ...
  const paragraphFromRows=(rows=[])=>{
    ...
    return joined?{text:joined.slice(0,1800),claimIds:rows.map(row=>row.id).filter(Boolean)}:null;
  };
  ...
  const issueSectionTarget=Math.min(6,Math.max(1,Math.ceil(issueRows.length/3)));
  const issueSectionRows=takeRows(issueRows,issueSectionTarget);
  const remainingForContext=[...issueRows,...profileRows].filter(row=>row?.id&&!usedFactIds.has(row.id));
  const contextSectionTarget=Math.min(5,Math.max(1,Math.ceil(remainingForContext.length/2)));
  const contextSectionRows=takeRows(remainingForContext,contextSectionTarget);
  const checkpointRows=takeRows([...issueRows,...profileRows],5);
  const sections=[];
