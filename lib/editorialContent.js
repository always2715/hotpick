...
export function buildVerifiedFallback(topicTitle, ledger={}, sourceWindowHours=36, contentTier='standard') {
  ...
  const paragraphFromRows=(rows=[])=>rows.length?{
    text:rows.map(row=>row.editorialText).filter(Boolean).join(' ').slice(0,520),
    claimIds:rows.map(row=>row.id).filter(Boolean),
  }:null;

  const identitySeed=(!profileRows.length&&role&&issueRows[0])
    ? [{...issueRows[0],editorialText:`${topicTitle}${topicParticle} ${role}입니다.`}]
    : [];
  const profileSectionRows=profileRows.length?takeRows(profileRows,2):identitySeed;
  const issueSectionRows=takeRows(issueRows,2);
  const contextSectionRows=takeRows([...issueRows,...profileRows],2);
  const checkpointRows=takeRows([...issueRows,...profileRows],1);
  const sections=[];
  ...
  if(checkpointRows.length){
    sections.push({
      heading:editorialContextHeading(checkpointRows),
      paragraphs:[paragraphFromRows(checkpointRows)].filter(Boolean),
    });
  }
  if(!sections.length){
  ...
  const pkg={
    ...
    accuracyProjection:'fact_ledger_literal_v3_editorial',
  };
