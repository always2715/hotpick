...
const FRAGMENT = /(?:^|\s)(?:do|the|for|of|with|vs|part|episode|ep)\s*$/i;
...
export function researchCandidateRejectionReasons(item={}) {
  ...
  const topic=clean(item.topTopic||'');
  if(!keyword||keyword.length<2||FRAGMENT.test(keyword)||FRAGMENT.test(title))out.push('검색어 또는 제목이 불완전함');
...
export function selectVisibleTrendPool(candidates=[], {limit=30}={}) {
  return (Array.isArray(candidates)?candidates:[])
    ...
    .slice(0,Math.max(1,Math.min(90,Number(limit||30))));
}
