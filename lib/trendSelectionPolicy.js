...
export function researchCandidateRejectionReasons(item={}) {
  ...
  const topic=clean(item.topTopic||'');
  if(!keyword||keyword.length<2||FRAGMENT.test(keyword)||FRAGMENT.test(title)||EDITORIAL_LABEL.test(keyword)||EDITORIAL_LABEL.test(title))out.push('검색어 또는 제목이 불완전함');
