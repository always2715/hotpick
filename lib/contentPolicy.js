...
export function collectOutputTexts(content = {}, options = {}) {
  const scope=typeof options==='string'?options:String(options?.scope||'all');
  ...
  const auxiliaryRows=scope==='feed'?[]:[
    ...(Array.isArray(content.qa) ? content.qa.flatMap(x => [x?.q,x?.a]) : []),
    ...(Array.isArray(content.instagramCards) ? content.instagramCards.flatMap(x => [x?.headline,x?.body]) : []),
    content.instagramCaption,
  ];
  return [...feedRows,...auxiliaryRows].map(cleanText).filter(Boolean);
}

export function assessCopyrightRisk(content = {}, sourceItems = [], options = {}) {
  const outputs = collectOutputTexts(content,options);
...
export function assessGrounding(content = {}, ledger = {}, options = {}) {
  const scope=typeof options==='string'?options:String(options?.scope||'all');
  ...
  if(scope!=='feed')(Array.isArray(content.qa)?content.qa:[]).forEach(row=>rows.push({text:row?.a,claimIds:row?.claimIds||[],kind:'qa'}));
  if(scope!=='feed')(Array.isArray(content.instagramCards)?content.instagramCards:[]).filter(row=>!['promo','source'].includes(row?.type)).forEach(row=>rows.push({text:row?.body,claimIds:row?.claimIds||[],kind:'instagram'}));
  const card=content.card||{};
...
export function decidePublication({ content, sourceItems = [], ledger = {}, qualityScore = 0, category = '' } = {}) {
  ...
  const feedFirst=Number(content?.contentVersion||0)>=140||String(content?.feedDetailLengthPolicy||'').includes('v8056-public-text');
  const validationScope=feedFirst?{scope:'feed'}:{};
  const copyright=assessCopyrightRisk(content,sourceItems,validationScope);
  const grounding=assessGrounding(content,ledger,validationScope);
  ...
  const deterministicFallback=String(content?.aiStatus||'').startsWith('verified_');
