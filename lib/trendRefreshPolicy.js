...
export function assessTrendSetHealth(next = [], previous = [], diagnostics = {}, { consecutiveLow = 0, targetCount = PUBLIC_TOP_COUNT } = {}) {
  const nextCount=Array.isArray(next)?next.length:0;
  const previousCount=Array.isArray(previous)?previous.length:0;
  const target=Math.max(1,Number(targetCount||PUBLIC_TOP_COUNT));
...
export function shouldCommitProgressiveRecovery(nextCount=0, previousCount=0, targetCount=PUBLIC_TOP_COUNT) {
...
export function mergeRecentPreviousTrends(next=[],previous=[],limit=PUBLIC_TOP_COUNT) {
  const result=[];
  const seen=new Set();
  for(const item of [...(Array.isArray(next)?next:[]),...(Array.isArray(previous)?previous:[])]){
    ...
    if(result.length>=(Math.max(1,Math.min(PUBLIC_TOP_COUNT,Number(limit||PUBLIC_TOP_COUNT)))))break;
