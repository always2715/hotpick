...
// v8.0.32: 실행별 콘텐츠를 publication_stage 한 곳에만 의존하지 않습니다.
...
export async function saveTrendRunContentSnapshot(runId, candidateId, content, options = {}) {
  ...
  const dualWrite=await writeDualRunSnapshot(r,{
    stageKey:publicationStageKey(stageId),snapshotKey,candidateId:safeCandidateId,serialized,
...
export async function getTrendRunContentSnapshot(runId, candidateId, options = {}) {
  ...
  for(let attempt=0;attempt<=retries;attempt++){
    try{
      ...
      const content=parse(await r.hget(cronRunContentKey(safeRunId),safeCandidateId),null);
