...
export async function getContent(slug, { includePrivate = false } = {}) {
  try {
    ...
    const persistedPublished=isPersistedPublishedContentForFeed(merged);
    if (!includePrivate && ([VISIBILITY.PRIVATE, VISIBILITY.TRASHED].includes(visibility) || (!isPublicContentReady(merged)&&!persistedPublished))) return null;
...
function buildGuaranteedCurrentTopFeedItem(content={},trend={},now=new Date().toISOString()){
  ...
  const summary=groundedFeedSummary({...trend,...content,card:merged.card})||compactGroundedFeedText(trend?.previewSummary||trend?.listSummary||trend?.previewWhy||'');
  if(!merged.slug||title.length<4||summary.length<20)return null;
...
async function loadCurrentTopSnapshotFeedRows(r,options={}){
  ...
  for(const trend of Array.isArray(topRows)?topRows.slice(0,PUBLIC_TOP_COUNT):[]){
    ...
    const summary=groundedFeedSummary(trend)||compactGroundedFeedText(trend.previewSummary||trend.listSummary||trend.previewDetailSummary||trend.previewWhy||'');
    if(title.length<4||summary.length<20)continue;
...
async function sourceOfTruthFeedFallback(r,options={}){
  ...
  const [topContentRows,topSnapshotRows,legacyRaw]=await Promise.all([
  ...
  // 현재 TOP 상세 원본을 최우선으로 사용하고, 누적 콘텐츠·TOP 스냅샷·레거시 피드를 차례로 보완합니다.
  [...topContentRows,...rows,...topSnapshotRows,...legacyRows].forEach(item=>{if(item?.slug&&!bySlug.has(item.slug))bySlug.set(item.slug,item);});
...
export async function getFeedPosts(limit=50,offset=0,options={}){
  ...
  try{
    ...
    if(!rows.length||currentTopExpected){
...
export async function queryFeedPosts({limit=20,offset=0,category='all',scope='all',sort='latest',search='',topSlugs=[],includeHidden=false}={}){
  ...
  try{
    ...
    return{items:all.slice(offset,offset+limit),total:all.length,recovered,errorCode:recovered?'FEED_SOURCE_OF_TRUTH_RECOVERED':''};
  }catch(error){
...
export async function saveThumbnailPoolState(state={}) {
  ...
  const targetSize=Number(state.targetSize||100);
  const normalized={
    version:String(state.version||'v1-100-curated'),targetSize,items,
...
export async function rebuildMissingTopFeeds({force=true}={}) {
  ...
  for (const trend of trends.slice(0,PUBLIC_TOP_COUNT)) {
    ...
    if (content.status === CONTENT_STATUS.PUBLISHED && ![VISIBILITY.HIDDEN_FEED,VISIBILITY.PRIVATE,VISIBILITY.TRASHED].includes(String(content.visibility||VISIBILITY.PUBLISHED)) && (isPublicContentReady({...content,status:CONTENT_STATUS.PUBLISHED,visibility:VISIBILITY.PUBLISHED})||persistedReady)) {
