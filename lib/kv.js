...
const K = {
  ...
  feedCumulativeMigration: `${NS}:feed:cumulative:migration:v8044`,
...
async function scanPersistedContentSlugs(r,{force=false,limit=5000}={}){
  ...
  const marker=parse(await r.get(K.feedCumulativeMigration).catch(()=>null),{})||{};
  ...
  await r.set(K.feedCumulativeMigration,JSON.stringify({discovered:discovered.length,scanned,updatedAt:new Date().toISOString()}),{ex:60*60*24}).catch(()=>{});
  return discovered;
}
...
export async function rebuildMissingTopFeeds({force=true}={}) {
  ...
  const result={ total:slugs.length, discovered:discovered.length, created, needsContent, skipped, hidden, missing, visibleCount:visibleRows.length, visibleTopCount:visibleTop.length, force:Boolean(force), cumulative:true };
