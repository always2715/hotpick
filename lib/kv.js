...
async function loadCurrentTopSnapshotFeedRows(r,options={}){
  ...
  for(const trend of Array.isArray(topRows)?topRows.slice(0,PUBLIC_TOP_COUNT):[]){
    ...
    const summary=String(trend.previewSummary||trend.listSummary||trend.previewDetailSummary||trend.previewWhy||'').trim();
...
async function sourceOfTruthFeedFallback(r,options={}){
  ...
  const topSnapshotRows=await loadCurrentTopSnapshotFeedRows(r,options);
  const bySlug=new Map();
  [...rows,...topSnapshotRows].forEach(item=>{if(item?.slug&&!bySlug.has(item.slug))bySlug.set(item.slug,item);});
  rows=[...bySlug.values()];
  if(rows.length){
