...
function publicSourceItems(items=[]){
  ...
  return (Array.isArray(items)?items:[]).filter(item=>allowedTypes.has(String(item?.sourceType||''))).map(item=>({
    title:item?.label||item?.displayLabel||item?.title||'참고 자료',label:item?.label||item?.displayLabel||item?.title||'참고 자료',link:item?.link||item?.canonicalUrl||'',source:item?.source||'',
  ...
  })).filter(item=>item.link).slice(0,8);
}
function publicRelatedItems(items=[]){
  return (Array.isArray(items)?items:[]).map(item=>({type:item?.type||'reference',source:item?.source||'',label:item?.label||'관련 자료',link:item?.link||'',domain:item?.domain||'',publishedAt:item?.publishedAt||null,date:item?.date||null})).filter(item=>item.link).slice(0,8);
}
...
async function ensureFeedIndexes(r){
  ...
  const topSlugs=(Array.isArray(topRows)?topRows:[]).map(row=>row?.slug).filter(Boolean).slice(0,30);
...
export async function saveTrends(trends) {
  ...
  if (!Array.isArray(trends) || trends.length !== 30) { const error=new Error(`TOP 저장에는 정확히 30개가 필요합니다. 현재 ${Array.isArray(trends)?trends.length:0}개입니다.`); error.code='SAVE_TRENDS_REQUIRES_TOP30'; throw error; }
  ...
  if (ranked.length !== 30 || new Set(ranked.map(item=>item.slug)).size !== 30) throw new Error('유효하고 중복되지 않은 TOP30이 아닙니다.');
...
export async function commitAtomicTopPublication(trends=[],contents=[]) {
  ...
  if(!Array.isArray(trends)||trends.length!==30){const error=new Error(`원자적 공개에는 정확히 30개의 TOP이 필요합니다. 현재 ${Array.isArray(trends)?trends.length:0}개입니다.`);error.code='ATOMIC_PUBLICATION_REQUIRES_TOP30';throw error;}
  if(!Array.isArray(contents)||contents.length!==30){const error=new Error(`원자적 공개에는 정확히 30개의 상세 콘텐츠가 필요합니다. 현재 ${Array.isArray(contents)?contents.length:0}개입니다.`);error.code='ATOMIC_PUBLICATION_REQUIRES_30_CONTENTS';throw error;}
  ...
  if(new Set(ranked.map(item=>item.slug)).size!==30){const error=new Error('중복 slug가 있어 TOP30 원자적 공개를 중단했습니다.');error.code='ATOMIC_PUBLICATION_DUPLICATE_SLUG';throw error;}

  const currentTop=parse(await r.get(K.trends),[])||[];
  if(currentTop.length===30)await r.set(K.stableTrends,JSON.stringify(currentTop));
  ...
  tx.set(K.feedIndexSchema,JSON.stringify({version:FEED_INDEX_SCHEMA_VERSION,trendsUpdatedAt:now,rebuiltAt:now,topOnly:true,checked:30,verified:30}));
  await tx.exec();
  ...
  if(feedPublishedCount!==30){
    const error=new Error(`TOP30 공개 후 피드 인덱스가 ${feedPublishedCount}건만 확인됐습니다.`);
...
export async function getCachedTrends(options = {}) {
  try {
    ...
    if(includeHidden){
      const adminRows=await normalizeAndFilter(latest.length?latest:stable);
      return adminRows.map((item,index)=>({...item,rank:index+1}));
    }

    let selected=latest.length===30?latest:(stable.length===30?stable:latest);
    let out=await normalizeAndFilter(selected);
    if(out.length!==30&&stable.length===30&&selected!==stable){
      const stableOut=await normalizeAndFilter(stable);
      if(stableOut.length===30)out=stableOut;
    }
...
export async function clearTrendRunWorkspace(runId) {
  try {
    const r = getRedis();
    if (!r || !runId) return;
    await r.del(cronRunCandidatesKey(runId));
  } catch {}
}
...
export async function clearGenerationHistory() {
  ...
  for (const id of ids) {
    if (!id) continue;
    keys.push(cronRunKey(id), cronRunTasksKey(id), cronRunCandidatesKey(id));
  }
...
export async function rebuildMissingTopFeeds({force=true}={}) {
  ...
  const topSlugs=trends.slice(0,30).map(item=>item.slug).filter(Boolean);
  const visibleRows=await getFeedPosts(1000,0,{scope:'top',topSlugs});
  const result={ total:trends.slice(0,30).length, created, needsContent, skipped, missing, visibleCount:visibleRows.length, force:Boolean(force) };
