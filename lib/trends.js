...
import { resolveEditorialKeyword, resolveTop30Keyword, validateTopCandidateKeyword } from './editorialTitle.js';
import { PUBLIC_TOP_COUNT, TOP_GENERATION_POOL_COUNT } from './topConfig.js';

export const TOP_TARGET_COUNT = TOP_GENERATION_POOL_COUNT;
...
export async function generateTop30(fetchTrendSignal, fetchYoutubeVideosFull, previousRanks = {}, options = {}) {
  ...
  const candidates = await mapLimit(merged, 5, async (item) => {
    ...
    const keywordQuality=validateTopCandidateKeyword(canonicalKeyword);
    ...
    return {
      ...
      topTitleSource:'keyword_only_pre_research',titleStatus:'pending_research',titleReady:false,researchTopicHint:neutral.topTopic,keywordSource:keywordResolution.source||null,keywordConfidence:Number(keywordResolution.score||0),keywordCandidates:(keywordResolution.candidates||[]).slice(0,5),keywordUsable:Boolean(keywordResolution.ok&&keywordQuality.valid),keywordValidationReasons:[...(keywordResolution.reasons||[]),...(keywordQuality.reasons||[])],topTopicSupport:neutral.topTopicSupport||0,topTitleConfidence:0,titleValidationReasons:neutral.titleValidationReasons||[], displayTitle:canonicalKeyword, searchQuery:canonicalSearchQuery,
  ...
  return { trends, researchPool, candidates:report, diagnostics:{
    generationPoolCount:TOP_TARGET_COUNT,publicTopCount:PUBLIC_TOP_COUNT,researchPoolLimit:TOP_RESEARCH_POOL_LIMIT,rawCollectionTarget:'150-250',selection:selection.diagnostics,
