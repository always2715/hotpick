...
import { createCronRun, patchCronRun, initializeCronRunTasks, addAudit, getCronRun } from './kv';
...
export async function enqueueTrendRefreshStep({ runId, trigger = 'external_cron', phase = 'start', cursor = 0 } = {}) {
  ...
  const queueGeneration=Math.max(0,Number(run?.queueGeneration||0));
  const response = await getQStashClient().publishJSON({
    ...
    deduplicationId: dedupeId(['update-trends-v837', runId, `g${queueGeneration}`, cleanPart(trigger)||'external_cron', safePhase, String(safeCursor)]),
    retries: 4,
    timeout: '300s',
    flowControl: {
