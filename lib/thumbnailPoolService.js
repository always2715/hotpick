...
function categoryQueries(config={}){
  ...
  return [...new Set([base,`${base} editorial background`,`${base} professional news background`].filter(Boolean))];
}
...
export async function bootstrapThumbnailPool({force=false,fast=false}={}){
  ...
  // 자동 준비는 카테고리별 1회 요청을 병렬 실행합니다. 최대 10회 요청이 한 번의 8초 예산 안에서 끝나도록 해 TOP 작업을 오래 막지 않습니다.
  const firstPass=await Promise.all(entries.map(async entry=>{
    const query=categoryQueries(entry.config)[0];
    try{return {...entry,photos:await fetchCategoryPhotos(query,1)};}
  ...
  if(!fast){
    for(const {category,config,seeds:categorySeeds} of entries){
      ...
      const attempts=[[queries[0],2],[queries[1],1],[queries[2],1]].filter(([query])=>query);
