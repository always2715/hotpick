...
import { FEED_DETAIL_MIN_CHARS, feedDetailLength } from './feedLengthPolicy.js';
...
export function publicContentRejectionReasons(content={}) {
  ...
  const feedBodyLength=feedDetailLength(content);
  const enforceFlexibleFeedLength=Number(content?.contentVersion||0)>=134||['v8044-2000-3000','v8045-min1000-target5000','v8046-min1000-target5000-recovery','v8056-public-text-min1000-target5000-feed-first'].includes(String(content?.feedDetailLengthPolicy||''));
  if(enforceFlexibleFeedLength&&feedBodyLength<FEED_DETAIL_MIN_CHARS)reasons.push(`피드 공개 텍스트 최소 ${FEED_DETAIL_MIN_CHARS.toLocaleString()}자 기준 미충족`);
  if(grade==='A'||grade==='B'){
