import { sanitizePublicContent } from './publicCopy.js';
...
export function publicContentRejectionReasons(content={}) {
  ...
  if(strictAccuracy&&facts.some(fact=>isGenericFactText(fact?.text||fact?.claim||'')))reasons.push('실제 사실문 대신 일반화된 Fact 문구가 포함됨');
...
// TOP30은 먼저 확정하고, 그 30개에 대해 독립 AI 조사·상세 작성·제목 생성을 수행합니다.
// 이 검사는 관리자 검토 플래그나 편집 진단 때문에 이미 선정된 TOP 키워드가 탈락하지 않도록
// 실제 공개에 필요한 구조·근거·제목·치명적 복제 위험만 확인합니다.
export function fixedTop30ContentRejectionReasons(content={}) {
...
export function isFixedTop30ContentReady(content={}) {
  return fixedTop30ContentRejectionReasons(content).length===0;
}
...
// v8.0.25: TOP30에 선정된 키워드는 '현재 사건 1건'이 반드시 있어야만 작성할 수 있는
...
export function fixedKeywordFeedRejectionReasons(content={}) {
  const reasons=[...fixedTop30ContentRejectionReasons(content)];
  const facts=(Array.isArray(content?.factLedger?.facts)?content.factLedger.facts:[])
  ...
  if(String(content?.contentGrade||'').toUpperCase()==='D')reasons.push('공통 관심 증가형 콘텐츠는 TOP30 피드로 사용할 수 없음');
