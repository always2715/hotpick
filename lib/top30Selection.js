...
function sourceUrls(item={}){
  const rows=[...(item.relatedArticles||[]),...(item.articles||[]),...(item.relatedNews||[]),...(item.sourceItems||[])];
  return new Set(rows.map(row=>canonicalSourceUrl(row?.link||row?.url||row?.canonicalUrl||'')).filter(Boolean));
}
function hasSharedSource(left={},right={}){
  const a=sourceUrls(left),b=sourceUrls(right);
...
export function isLooseSameCandidate(left={},right={}) {
  if(left?.eventKey&&right?.eventKey&&String(left.eventKey)===String(right.eventKey))return true;
  if(left?.stableEventId&&right?.stableEventId&&String(left.stableEventId)===String(right.stableEventId))return true;
  if(hasSharedSource(left,right))return true;
...
function withCaps(rows=[],limit=TOP_GENERATION_POOL_COUNT,caps=null){
...
export function selectStableTop30(candidates=[], {limit=TOP_GENERATION_POOL_COUNT}={}) {
  const target=Math.max(PUBLIC_TOP_COUNT,Math.min(TOP_GENERATION_POOL_COUNT,Number(limit||TOP_GENERATION_POOL_COUNT)));
