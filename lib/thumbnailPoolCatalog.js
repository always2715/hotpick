export const THUMBNAIL_POOL_VERSION = 'v2-500-curated';
export const THUMBNAIL_POOL_TARGET_SIZE = 500;
...
const COMPOSITION_VARIANTS = Object.freeze([
  {key:'base',label:'기본 구도',query:''},
  {key:'wide',label:'넓은 전경',query:'wide angle editorial landscape'},
  {key:'detail',label:'상징적 디테일',query:'professional detail close up'},
  {key:'night',label:'차분한 야간',query:'night editorial background'},
  {key:'minimal',label:'미니멀 배경',query:'minimal clean news background'},
]);

function seedFromSlot(category,config,slot,id,variant=COMPOSITION_VARIANTS[0]){
  const [moodTitle,moods,subjects,tone]=slot;
  return {
    id:`${category}_${String(id).padStart(3,'0')}`,
    category,
    categoryLabel:config.label,
    query:[config.query,variant.query].filter(Boolean).join(' '),
    moodTitle:variant.key==='base'?moodTitle:`${moodTitle} · ${variant.label}`,
    moods:[...moods],
    subjects:[...subjects],
    tone,
    composition:variant.key,
...
export function buildThumbnailPoolSeeds(){
  ...
  for(const [category,config] of Object.entries(THUMBNAIL_POOL_CATEGORIES)){
    // 001~010은 기존 100개 풀 ID와 의미를 그대로 유지합니다.
    config.slots.forEach((slot,index)=>result.push(seedFromSlot(category,config,slot,index+1,COMPOSITION_VARIANTS[0])));
    // 카테고리마다 추가 40개를 서로 다른 구도로 확장해 총 50개씩, 전체 500개를 구성합니다.
    COMPOSITION_VARIANTS.slice(1).forEach((variant,variantIndex)=>{
      config.slots.forEach((slot,index)=>{
        const id=11+(variantIndex*config.slots.length)+index;
        result.push(seedFromSlot(category,config,slot,id,variant));
      });
    });
  }
  return result;
